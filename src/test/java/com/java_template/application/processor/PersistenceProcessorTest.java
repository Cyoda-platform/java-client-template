...
@DisplayName("PersistenceProcessor Tests")
class PersistenceProcessorTest {

    private PersistenceProcessor processor;
    ...
    void setUp() {
        ...
        processor = new PersistenceProcessor(serializerFactory, entityService);
    }
    ...
    void testSupports() {
        ...
        OperationSpecification.Processor opSpec = new OperationSpecification.Processor(
                modelSpec, "Order", "NEW", "VALIDATE", "OrderLifecycle");
    ...
    void testDoesNotSupportDifferentOperation() {
        ModelSpec modelSpec = new ModelSpec().withName("Order").withVersion(1);
        OperationSpecification.Processor opSpec = new OperationSpecification.Processor(
                modelSpec, "DifferentProcessor", "NEW", "VALIDATE", "OrderLifecycle");

        assertFalse(processor.supports(opSpec));
    }

    @Test
    @DisplayName("Should persist valid order successfully")
    void testPersistValidOrder() {
    ...
    @Test
    @DisplayName("Should handle persistence of order with null fields")
    void testPersistOrderWithNullFields() {
    ...
    private EntityProcessorCalculationRequest createRequest(Order order) {
        ...
        metadataJson.put("state", "VALIDATED");

...
@DisplayName("ExecutionReportProcessor Tests")
class ExecutionReportProcessorTest {
    ...
    void setUp() {
        ...
        processor = new ExecutionReportProcessor(serializerFactory, entityService);
    }
    ...
    void testSupports() {
        ...
        OperationSpecification.Processor opSpec = new OperationSpecification.Processor(
                modelSpec, "Order", "ACKNOWLEDGED", "FILL", "OrderLifecycle");
    ...
    void testDoesNotSupportDifferentOperation() {
        ModelSpec modelSpec = new ModelSpec().withName("Order").withVersion(1);
        OperationSpecification.Processor opSpec = new OperationSpecification.Processor(
                modelSpec, "DifferentProcessor", "ACKNOWLEDGED", "FILL", "OrderLifecycle");

        assertFalse(processor.supports(opSpec));
    }

    @Test
    @DisplayName("Should process execution report successfully")
    void testProcessExecutionReportSuccess() {
    ...
    @Test
    @DisplayName("Should handle partial fill execution report")
    void testProcessPartialFillReport() {
        Order order = createValidOrder();
        EntityProcessorCalculationRequest request = createRequest(order);
        CyodaEventContext<EntityProcessorCalculationRequest> context = createContext(request);
    ...
    private EntityProcessorCalculationRequest createRequest(Order order) {
        ...
        request.setProcessorName("ExecutionReportProcessor");
        ...
        metadataJson.put("state", "ACKNOWLEDGED");

...
public class ReserveStockProcessorTest {

    @Test
    public void testProcess_reserveStockSuccess() throws Exception {
        ...
        ReserveStockProcessor processor = new ReserveStockProcessor(factory, entityService, objectMapper);
        ...
        Item it = new Item(); it.setProductId("p1"); it.setQuantity(2); it.setUnitPrice(5.0);
        items.add(it);
        ...
        prodNode.put("technicalId", java.util.UUID.randomUUID().toString());
        prodNode.put("productId", "p1");
        prodNode.put("name", "Prod");
        prodNode.put("sku", "sku1");
        prodNode.put("price", 5.0);
        prodNode.put("stockQuantity", 10);
        results.add(prodNode);
        ...
        // Mock updateItem to return completed future
        when(entityService.updateItem(eq(Product.ENTITY_NAME), eq(String.valueOf(Product.ENTITY_VERSION)), any(), any())).thenReturn(CompletableFuture.completedFuture(java.util.UUID.randomUUID()));

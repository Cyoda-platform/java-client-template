...
public class CreateOpportunityProcessorTest {

    @Test
    public void testProcess_createsOpportunityAndUpdatesLead() throws Exception {
        ...
        CreateOpportunityProcessor processor = new CreateOpportunityProcessor(sf, entityService, null);
        ...
        Method m = CreateOpportunityProcessor.class.getDeclaredMethod("processEntityLogic", ProcessorSerializer.ProcessorEntityExecutionContext.class);
        m.setAccessible(true);
        Lead result = (Lead) m.invoke(processor, ctx);

        assertNotNull(result);
        assertEquals("OPPORTUNITY_CREATED", result.getStatus());
        // verify that addItem was called for Opportunity
        verify(entityService).addItem(eq(Opportunity.ENTITY_NAME), eq(String.valueOf(Opportunity.ENTITY_VERSION)), any());
        // verify lead update called

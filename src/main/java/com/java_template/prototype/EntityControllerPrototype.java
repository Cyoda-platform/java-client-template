...
public class EntityControllerPrototype {
    ...
    // In-memory fact sent count
    private int factSentCount = 0;

    // In-memory interaction counts: interactionType -> count
    private final Map<String, Integer> interactionCounts = new ConcurrentHashMap<>();

    // In-memory sent cat fact id store (for interaction reference)
    // For simplicity, the factId is generated as UUID string on sendWeekly
    private String lastFactId = null;
    private String lastFactText = null;
    ...
    public ResponseEntity<SendWeeklyResponse> sendWeeklyCatFact() {
        ...
        // Generate fact ID for this fact send
    ...
    public ResponseEntity<GenericResponse> recordInteraction(@RequestBody @Validated InteractionRequest request) {
        ...
        if (lastFactId == null || !lastFactId.equals(request.getFactId())) {
            ...
            // Still accept interaction, just log warning
        }
    ...
    @Data
    static class ReportSummaryResponse {
    ...
    static class InteractionRequest {
        ...
        @NotBlank
        private String interactionType; // e.g. email_open, link_click
    }

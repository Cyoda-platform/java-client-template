...
public class CyodaEntityControllerPrototype {
    ...
    public CompletableFuture<ResponseEntity<GetPetsResponse>> getPets() {
        ...
        return entityService.getItems(
                ...
                List<Pet> pets = arrayNode.findValuesAsText("technicalId").isEmpty()
                    ? List.of()
                    : arrayNode.findValuesAsText("technicalId").isEmpty()
                        ? List.of()
    ...
    public CompletableFuture<ResponseEntity<AdoptPetResponse>> adoptPet(@RequestBody @Valid AdoptPetRequest request) {
        UUID technicalId = UUID.nameUUIDFromBytes(request.getPetId().toString().getBytes());
        // We need to find pet by id, but EntityService uses technicalId.
        // So we search by condition on pet id field (assuming it's stored as 'id' or 'petId' in entity)

...
public class CyodaEntityControllerPrototype {
    ...
    // Minor/utility entities, keep local cache
    ...
    // Core business entity: Subscriber is minor, keep local cache

    // Core business entity: GameScore - refactor to use EntityService

    // Create a DTO for GameScore with technicalId ignored for input
    @Data
    ...
    public CompletableFuture<ResponseEntity<Map<String, Object>>> getAllGames(
            @RequestParam(defaultValue = "20") @Min(1) @Max(100) int limit,
            @RequestParam(defaultValue = "0") @Min(0) int offset) {
        logger.info("Retrieving all games with limit {} and offset {}", limit, offset);
        return entityService.getItems("GameScore", ENTITY_VERSION)
                .thenApply(arrayNode -> {
                    List<GameScore> allGames = new ArrayList<>();
                    arrayNode.forEach(node -> {
                        GameScore g = readGameScoreFromNode((ObjectNode) node);
                        allGames.add(g);
                    });
                    allGames.sort(Comparator.comparing(GameScore::getDate, Comparator.nullsLast(Comparator.reverseOrder())));
                    int total = allGames.size();
                    int from = Math.min(offset, total);
                    int to = Math.min(offset + limit, total);
                    List<GameScore> page = allGames.subList(from, to);
                    return ResponseEntity.ok(Map.of("games", page, "limit", limit, "offset", offset, "total", total));
                });
    }

    @GetMapping("/games/{date}")
    public CompletableFuture<ResponseEntity<List<GameScore>>> getGamesByDate(
        ...
        return entityService.getItemsByCondition("GameScore", ENTITY_VERSION, conditionRequest)
                .thenApply(arrayNode -> {
    ...
    private void fetchStoreAndNotify(LocalDate date) throws Exception {
        ...
        if (!root.isArray()) {
            throw new ResponseStatusException(HttpStatus.BAD_GATEWAY, "Invalid data received from NBA API");
        }

        List<GameScore> parsed = new ArrayList<>();
        List<GameScore> toAdd = new ArrayList<>();
        ...
        // Add items to entity service
        CompletableFuture<List<UUID>> idsFuture = entityService.addItems("GameScore", ENTITY_VERSION, toAdd);
        idsFuture.join(); // wait synchronously here for simplicity, alternatively make fetchStoreAndNotify async and await properly

        sendEmailNotifications(dateStr, toAdd);
    }
    ...
    private GameScore readGameScoreFromNode(ObjectNode node) {
        GameScore g = new GameScore();
        JsonNode technicalIdNode = node.get("technicalId");
        if (technicalIdNode != null && !technicalIdNode.isNull()) {
            g.setTechnicalId(UUID.fromString(technicalIdNode.asText()));
        }
        g.setDate(getText(node, "date"));
        g.setHomeTeam(getText(node, "homeTeam"));
        g.setAwayTeam(getText(node, "awayTeam"));
        g.setHomeScore(getInt(node, "homeScore"));
        g.setAwayScore(getInt(node, "awayScore"));
        return g;
    }

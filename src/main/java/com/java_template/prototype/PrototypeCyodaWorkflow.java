...
public class CyodaEntityControllerPrototype {
    ...
    /**
     * Workflow function that processes the Pet entity before persistence.
     * You can modify the pet object here, add/get other entities (different entityModel),
     * but do NOT add/update/delete the same entityModel ("pet") to avoid infinite recursion.
     */
    private Pet processPet(Pet pet) {
        // Example: Ensure description is never null or empty before persistence
        if (pet.getDescription() == null || pet.getDescription().isBlank()) {
            pet.setDescription("A lovely pet!");
        }
        ...
        if (request.getPetId() != null && !request.getPetId().isBlank()) {
            ...
            if (existingPetNode == null || existingPetNode.isEmpty()) {
                ...
                try {
                    ...
                    // create in entityService with workflow
                    CompletableFuture<UUID> addedIdFuture = entityService.addItem(
                            ENTITY_NAME,
                            ENTITY_VERSION,
                            pet,
                            this::processPet
...
- Added a new private method `processPet(Pet pet)` which implements the required workflow function that processes the entity before persistence.
- Updated all calls to `entityService.addItem` to pass the workflow function as the last argument: `this::processPet`.
- The workflow function currently ensures that the `description` field is never null or blank before saving.
- Updated imports to include `java.util.function.Function` for clarity (though method reference `this::processPet` works fine).
- The rest of the code remains unchanged, except usage of the updated `addItem` signature.
...
public class EntityControllerPrototype {
    ...
    public ResponseEntity<SearchResultResponse> searchFlights(@RequestBody @Valid SearchRequest request) {
        log.info("Received flight search request: {} -> {} on {}, passengers {}",
        ...
        CompletableFuture.runAsync(() -> processFlightSearch(request, searchId))
                .exceptionally(ex -> {
                    log.error("Error processing flight search for {}: {}", searchId, ex.getMessage());
                    return null;
                });
    ...
    public ResponseEntity<SearchResultResponse> getCachedResults(@PathVariable @NotBlank String searchId) {
        log.info("Retrieving cached flight search results for {}", searchId);
    ...
    void processFlightSearch(SearchRequest request, String searchId) {
        log.info("Processing flight search for {}", searchId);
        try {
            ...
            log.info("Finished processing {} flights for {}", flights.size(), searchId);
        } catch (Exception e) {
            log.error("Failed to process flight search for {}: {}", searchId, e.getMessage());
    ...
    public ResponseEntity<Map<String,String>> handleError(ResponseStatusException e) {
        log.error("Error {}: {}", e.getStatusCode(), e.getReason());

...
public class EntityControllerPrototype {
    ...
    // For simplicity: store weekly cat facts keyed by ISO week (e.g. "2024-W17")
    private final Map<String, CatFact> weeklyCatFacts = new ConcurrentHashMap<>();

    // Aggregate email interaction counts keyed by week string
    ...
    public ResponseEntity<String> subscribe(@RequestBody SubscriberRequest request) {
        ...
        // If previously unsubscribed, remove from unsubscribed map on resubscribe via this endpoint
    ...
    public ResponseEntity<String> unsubscribe(@RequestBody SubscriberRequest request) {
        String email = normalizeEmail(request.getEmail());
        validateEmail(email);
        ...
        unsubscribedSubscribers.put(email, removed); // keep history indefinitely

        // TODO: Send confirmation email asynchronously (fire-and-forget)
    ...
    public ResponseEntity<String> resubscribe(@RequestBody SubscriberRequest request) {
        String email = normalizeEmail(request.getEmail());
        validateEmail(email);
    ...
    // -----------------------------------
    // 2. Cat Fact Ingestion & Email Send
    // -----------------------------------

    @PostMapping(value = "/facts/ingest-and-send")
    public ResponseEntity<String> ingestAndSend() {
        ...
        try {
            ...
            // Fire-and-forget email sending to all active subscribers
    ...
    @PostMapping(value = "/facts/manual-send")
    ...
    @GetMapping(value = "/reports/subscribers-count", produces = MediaType.APPLICATION_JSON_VALUE)
    ...
    @GetMapping(value = "/reports/interaction-summary", produces = MediaType.APPLICATION_JSON_VALUE)
    public InteractionSummary getInteractionSummary(
            @RequestParam String startDate,
            @RequestParam String endDate) {
        // For prototype: return aggregated counts for current week or zero if missing
        String currentWeek = getCurrentIsoWeek();
    ...
    // -----------------------------------
    // 4. Email Interaction Webhook (Mock)
    // -----------------------------------

    @PostMapping(value = "/webhook/email-interaction", consumes = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<String> emailInteractionWebhook(@RequestBody EmailInteraction interaction) {
        logger.info("Received email interaction webhook: {}", interaction);

        // Aggregate interactions by current week - simplistic increment
        String currentWeek = getCurrentIsoWeek();
    ...
    private void sendEmailToAllSubscribers(CatFact catFact) {
        ...
        // TODO: Integrate with real email service, retry failed emails up to 3 times

        for (Subscriber subscriber : subscribers.values()) {
            // Fire-and-forget per email send (simulate)
            CompletableFuture.runAsync(() -> {
                try {
                    // TODO: Replace with real email sending logic
                    logger.info("Sending cat fact email to {}", subscriber.getEmail());
                    Thread.sleep(100); // Simulate latency
                    // On success, could update tracking data if needed

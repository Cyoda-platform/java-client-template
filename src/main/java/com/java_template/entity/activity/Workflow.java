...
public class Workflow {
    ...
    public DailyReportResponse getDailyReport(
        ...
        CompletableFuture<com.fasterxml.jackson.databind.node.ArrayNode> filteredItemsFuture = entityService.getItemsByCondition(
                ...
                ENTITY_VERSION,
                condition);
        com.fasterxml.jackson.databind.node.ArrayNode items = filteredItemsFuture.get();
    ...
    // Workflow orchestration only here. Calls processing steps sequentially.
    private CompletableFuture<ObjectNode> processactivity(JsonNode entityNode) {
        ObjectNode entity = (ObjectNode) entityNode;
        return processFetchData(entity)
                .thenCompose(this::processAnalyzeData)
                .thenCompose(this::processBuildSummary)
    ...
    private CompletableFuture<ObjectNode> processFetchData(ObjectNode entity) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                entity.set("rawData", rootNode);
                logger.info("processFetchData completed");
                return entity;
            } catch (Exception e) {
                logger.error("Error in processFetchData: {}", e.getMessage(), e);
    ...
    private CompletableFuture<ObjectNode> processAnalyzeData(ObjectNode entity) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                JsonNode rootNode = entity.get("rawData");
                ...
                entity.set("userActivityCount", objectMapper.valueToTree(userActivityCount));
                entity.set("activityTypeCount", objectMapper.valueToTree(activityTypeCount));
                logger.info("processAnalyzeData completed");
                return entity;
            } catch (Exception e) {
                logger.error("Error in processAnalyzeData: {}", e.getMessage(), e);
                throw new RuntimeException(e);
            }
        });
    }

    private CompletableFuture<ObjectNode> processBuildSummary(ObjectNode entity) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                Map<Integer, Integer> userActivityCount = objectMapper.convertValue(entity.get("userActivityCount"), Map.class);
                Map<String, Integer> activityTypeCount = objectMapper.convertValue(entity.get("activityTypeCount"), Map.class);

                int totalUsers = userActivityCount != null ? userActivityCount.size() : 0;
                int totalActivities = 0;
                if (userActivityCount != null) {
                    totalActivities = userActivityCount.values().stream().mapToInt(Integer::intValue).sum();
                }

                String mostFrequentActivity = "N/A";
                if (activityTypeCount != null && !activityTypeCount.isEmpty()) {
                    mostFrequentActivity = activityTypeCount.entrySet().stream()
                ...
                for (int uid = 1; uid <= 10; uid++) {
                    if (userActivityCount == null || !userActivityCount.containsKey(uid)) {
                ...
                logger.info("processBuildSummary completed");
                return entity;
            } catch (Exception e) {
                logger.error("Error in processBuildSummary: {}", e.getMessage(), e);
                throw new RuntimeException(e);
            }
        });
    }
    ...
    private CompletableFuture<ObjectNode> processFinalize(ObjectNode entity) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                // Ensure summaryDate is present
                String dateStr = entity.has("summaryDate") ? entity.get("summaryDate").asText() : LocalDate.now().toString();
                entity.put("summaryDate", dateStr);
                // Cleanup temporary fields
                entity.remove("rawData");
                entity.remove("userActivityCount");
                entity.remove("activityTypeCount");
                logger.info("processFinalize completed for date {}", dateStr);
                return entity;
            } catch (Exception e) {
                logger.error("Error in processFinalize: {}", e.getMessage(), e);
                throw new RuntimeException(e);
            }
        });
    }

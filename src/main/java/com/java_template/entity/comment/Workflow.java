...
public class CyodaEntityControllerPrototype {
    ...
    // Workflow class with separated methods
    public class Workflow {

        public CompletableFuture<ObjectNode> processcomment(Object entity) {
            ...
            return CompletableFuture.supplyAsync(() -> {
                try {
                    ObjectNode fetchedComments = processFetchComments(postId);
                    ObjectNode summaryNode = processAnalyzeComments(fetchedComments, postId);
                    processUpdateEntity(entityNode, summaryNode);
                    processSendEmail(email, postId, summaryNode);
        ...
        private CompletableFuture<ObjectNode> processFetchCommentsAsync(int postId) {
            return CompletableFuture.supplyAsync(() -> processFetchComments(postId));
        }

        private ObjectNode processFetchComments(int postId) {
            try {
                ...
                return (ObjectNode) objectMapper.readTree(jsonResponse);
            } catch (Exception ex) {
                throw new RuntimeException("Failed to fetch comments: " + ex.getMessage(), ex);
            }
        }

        private ObjectNode processAnalyzeComments(ObjectNode commentsNode, int postId) {
        ...
        private void processUpdateEntity(ObjectNode entityNode, ObjectNode summaryNode) {
            entityNode.put("analysisStatus", "completed");
            entityNode.set("summary", summaryNode);
            entityNode.put("lastUpdated", Instant.now().toString());
        }

        private void processSendEmail(String email, int postId, ObjectNode summaryNode) {
            CompletableFuture.runAsync(() -> {
                try {
                    logger.info("Sending email to {} for postId={} summary= {}", email, postId, summaryNode);
                } catch (Exception ex) {

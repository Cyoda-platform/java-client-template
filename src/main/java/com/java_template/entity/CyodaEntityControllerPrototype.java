...
public class CyodaEntityControllerPrototype {
    ...
    private void fetchAndStorePets(String sourceUrl, String status) {
        try {
            ...
            if (!petsToAdd.isEmpty()) {
                entityService.addItems(ENTITY_NAME, ENTITY_VERSION, petsToAdd).get();
            }
    ...
    public ResponseEntity<List<Pet>> searchPets(@RequestBody @Valid SearchRequest request) throws ExecutionException, InterruptedException {
        ...
        SearchConditionRequest conditionRequest = conditions.isEmpty() ?
                null : SearchConditionRequest.group("AND", conditions.toArray(new Condition[0]));
        ...
        for (JsonNode node : items) {
            Pet pet = jsonNodeToPet(node);
            if (pet != null) {
                // Because entityService stores unique id as technicalId, but Pet.id is Long and from original data
                // Here we keep pet.id as is, because original id is preserved in the entity
                results.add(pet);
            }
        }
    ...
    public ResponseEntity<Pet> getPetById(@PathVariable @NotNull @Positive Long petId) throws ExecutionException, InterruptedException {
        ...
        // Need to find entity by condition on id field, because we don't have technicalId here
    ...
    public ResponseEntity<Set<String>> getCategories() throws ExecutionException, InterruptedException {
        ...
        // Fetch all pets, collect categories uniquely

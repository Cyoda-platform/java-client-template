...
public class CyodaEntityControllerPrototype {
    ...
    private void fetchTransformStoreUsers() throws Exception {
        ...
        if (!usersToAdd.isEmpty()) {
            CompletableFuture<List<UUID>> idsFuture = entityService.addItems(ENTITY_NAME, ENTITY_VERSION, usersToAdd);
            idsFuture.join(); // wait for completion
            logger.info("Fetched and stored {} users via entityService", usersToAdd.size());
        }
    }

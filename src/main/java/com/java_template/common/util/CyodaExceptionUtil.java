...
public class CyodaExceptionUtil {
    ...
    /**
     * Checks if an exception represents a NOT_FOUND error from the Cyoda backend.
     * Walks the exception cause chain to find a StatusRuntimeException with NOT_FOUND status.
     *
     * @param exception the exception to check
     * @return true if the exception represents a NOT_FOUND error, false otherwise
     */
    public static boolean isNotFound(Throwable exception) {
        Throwable cause = exception;
        while (cause != null) {
            if (cause instanceof StatusRuntimeException) {

...
/**
 * Checkout Controller for OMS system
 * Handles anonymous checkout with guest contact information
 */
@RestController
    ...
    public ResponseEntity<EntityWithMetadata<Cart>> setGuestContact(
        ...
        try {
            ...
            if (!"CHECKING_OUT".equals(cart.getStatus())) {
                logger.error("Cart {} is not in CHECKING_OUT status: {}", cartId, cart.getStatus());
            ...
            logger.info("Guest contact set for cart {}: {}", cartId, guestContact.getName());

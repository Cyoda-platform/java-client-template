...
/**
 * OrderController - REST API for Order entity management
 * Handles order placement, matching, and lifecycle
 */
@RestController
...
public class OrderController {
    ...
    public ResponseEntity<PageResult<EntityWithMetadata<Order>>> getOrdersByAccountId(
        ...
        try {
            ...
            PageResult<EntityWithMetadata<Order>> result = entityService.search(
                    modelSpec, groupCondition, Order.class, params);

            logger.info("Found {} orders for account: {}", result.data().size(), accountId);
            return ResponseEntity.ok(result);
        } catch (Exception e) {
            ProblemDetail problemDetail = ProblemDetail.forStatusAndDetail(
                HttpStatus.BAD_REQUEST,
                String.format("Failed to retrieve orders: %s", e.getMessage())
            );
            return ResponseEntity.of(problemDetail).build();
        }
    }
    ...
    public ResponseEntity<EntityWithMetadata<Order>> cancelOrder(@PathVariable UUID id) {
        try {
            ...
            EntityWithMetadata<Order> response = entityService.update(id, current.entity(), "cancel");

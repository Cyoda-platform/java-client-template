 ...
 * Handles cart creation, item management, and checkout operations
 */
@RestController
...
public class CartController {
    ...
    public ResponseEntity<EntityWithMetadata<Cart>> updateCartItem(
        ...
        try {
            ...
            if (request.getQty() <= 0) {
                ...
                transition = request.getQty() > existingLine.getQty() ? "add_item" : "decrement_item";
            }
    ...
    /**
     * Open checkout (set cart to checking out state)
    ...
    public ResponseEntity<EntityWithMetadata<Cart>> openCheckout(@PathVariable String cartId) {
        try {
            ...
            // Update cart to checking out state
            EntityWithMetadata<Cart> response = entityService.update(
    ...
    /**
     * Complete checkout (attach guest contact)
     * POST /ui/checkout/{cartId}
     */
    @PostMapping("/checkout/{cartId}")
    public ResponseEntity<EntityWithMetadata<Cart>> completeCheckout(
            @PathVariable String cartId,

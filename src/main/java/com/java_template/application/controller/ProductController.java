...
/**
 * Product controller for catalog management in OMS
 * Provides product search, filtering, and CRUD operations
 */
@RestController
...
public class ProductController {
    ...
    public ResponseEntity<List<ProductSlimDto>> searchProducts(
        ...
        try {
            ...
            if (search != null && !search.trim().isEmpty()) {
                ...
                conditions.add(new SimpleCondition()); // Placeholder - we'll use the group condition
            }
            ...
            if (conditions.isEmpty() && (search == null || search.trim().isEmpty())) {
            ...
            logger.info("Found {} products with filters - search: {}, category: {}, price: {}-{}", 
     ...
     * PUT /ui/products/{sku}?transition=TRANSITION_NAME
     */
    @PutMapping("/{sku}")
    public ResponseEntity<EntityWithMetadata<Product>> updateProductBySku(
        ...
        try {
            // Ensure SKU matches
            product.setSku(sku);
            EntityWithMetadata<Product> response = entityService.updateByBusinessId(product, "sku", transition);
            logger.info("Product updated with SKU: {}", sku);
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            logger.error("Error updating product by SKU: {}", sku, e);
            return ResponseEntity.badRequest().build();
        }
    }

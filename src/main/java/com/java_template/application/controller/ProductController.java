 ...
 * and CRUD operations. Provides both slim DTOs for list views and full entities for details.
 */
@RestController
...
public class ProductController {
        ...
        try {
            ...
            if (conditions.isEmpty()) {
                ...
                List<ProductSlimDto> slimDtos = productPage.getContent().stream()
                        .map(this::toSlimDto)
                        .toList();
                return ResponseEntity.ok(new PageImpl<>(slimDtos, pageable, productPage.getTotalElements()));
            } else {
                // With filters - search and manually paginate
                GroupCondition groupCondition = new GroupCondition()
                        .withOperator(GroupCondition.Operator.AND)
                        .withConditions(conditions);
                products = entityService.search(modelSpec, groupCondition, Product.class);

                // Convert to slim DTOs and manually paginate
    ...
    /**
     * Get product by SKU (business ID) - returns full Product document
     * GET /ui/products/{sku}
     */
    @GetMapping("/{sku}")
     ...
     * PUT /ui/products/id/{id}?transition=update_inventory
     */
    @PutMapping("/id/{id}")

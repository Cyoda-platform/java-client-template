...
/**
 * ABOUTME: REST controller for Product endpoints supporting catalog browsing,
 * filtering by category and price range, and free-text search.
 */
@RestController
...
public class ProductController {
    ...
    public ResponseEntity<EntityWithMetadata<Product>> getProductBySku(@PathVariable String sku) {
        try {
            ...
            EntityWithMetadata<Product> product = entityService.findByBusinessIdOrNull(
            ...
            ProblemDetail problemDetail = ProblemDetail.forStatusAndDetail(
                    HttpStatus.BAD_REQUEST,
                    String.format("Failed to get product: %s", e.getMessage())
            );
            return ResponseEntity.of(problemDetail).build();
        }
    }

...
/**
 * Product Controller for OMS system
 * Handles product catalog operations including search, filtering, and detail views
 */
@RestController
...
public class ProductController {
    ...
    public ResponseEntity<List<ProductSlimDto>> searchProducts(
        ...
        try {
            ...
            logger.info("Found {} products (showing {} to {})", slimProducts.size(), start, end);
    ...
    /**
     * Get product detail by SKU (full document)
     * GET /ui/products/{sku}
     */
    @GetMapping("/{sku}")
    ...
    public ResponseEntity<EntityWithMetadata<Product>> createProduct(@RequestBody Product product) {
        try {
            ...
            creationEvent.setAt(LocalDateTime.now().toString());

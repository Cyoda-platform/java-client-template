...
/**
 * ABOUTME: REST controller for Product catalog operations including search, filtering,
 * CRUD operations, and inventory management for the OMS system.
 */
@RestController
...
public class ProductController {
    ...
    public ResponseEntity<Page<ProductSlimDto>> searchProducts(
        ...
        try {
            ...
            if (conditions.isEmpty()) {
                ...
                List<ProductSlimDto> slimProducts = productPage.getContent().stream()
                        .map(this::toSlimDto)
                        .toList();
                return ResponseEntity.ok(new PageImpl<>(slimProducts, pageable, productPage.getTotalElements()));
            } else {
                // With filters - get all and manually paginate
    ...
    /**
     * Update product
     * PUT /ui/products/{id}?transition=update_product
     */
    @PutMapping("/{id}")
     ...
     * POST /ui/products/{id}/inventory
     */
    @PostMapping("/{id}/inventory")
    public ResponseEntity<EntityWithMetadata<Product>> updateInventory(@PathVariable UUID id) {
        try {
            ModelSpec modelSpec = new ModelSpec().withName(Product.ENTITY_NAME).withVersion(Product.ENTITY_VERSION);
            EntityWithMetadata<Product> current = entityService.getById(id, modelSpec, Product.class);
            
            if (current == null) {
                return ResponseEntity.notFound().build();
            }

            EntityWithMetadata<Product> response = entityService.update(id, current.entity(), "update_inventory");
            logger.info("Product inventory updated for ID: {}", id);
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            logger.error("Failed to update inventory for product ID: {}", id, e);
            ProblemDetail problemDetail = ProblemDetail.forStatusAndDetail(
                HttpStatus.BAD_REQUEST,
                String.format("Failed to update inventory for product ID '%s': %s", id, e.getMessage())
            );
            return ResponseEntity.of(problemDetail).build();
        }
    }
    ...
    /**
     * Convert Product entity to slim DTO for list views
     */

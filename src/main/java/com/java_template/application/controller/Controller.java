...
public class Controller {
    ...
    @PostMapping
    public ResponseEntity<Map<String, String>> createMail(@Valid @RequestBody Mail mail) throws InterruptedException, ExecutionException, JsonProcessingException {
        try {
    ...
    public ResponseEntity<Mail> getMail(@PathVariable String technicalId) throws InterruptedException, ExecutionException, JsonProcessingException {
        try {
            ...
            logger.error("JSON processing error retrieving Mail entity for technicalId {}: {}", technicalId, e.getMessage());
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
        } catch (Exception e) {

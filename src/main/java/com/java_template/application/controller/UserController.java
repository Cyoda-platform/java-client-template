...
@Tag(name = "User API", description = "API for User entity (proxy to entity service)")
public class UserController {
    ...
    public ResponseEntity<?> createUser(@RequestBody(description = "User request payload") @org.springframework.web.bind.annotation.RequestBody UserRequest request) {
        try {
            ...
            user.setFavorites(request.favorites);
            user.setNotificationPreferences(request.notificationPreferences);
            user.setStatus(request.status != null ? request.status : "REGISTERED");
            ...
            URI location = URI.create(String.format("/users/%s", id.toString()));
            ...
            if (cause instanceof NoSuchElementException) {
                ...
                logger.error("Execution error creating user", e);
    ...
    @Operation(summary = "Get User by technicalId", description = "Retrieve a user by its technicalId")
    @ApiResponses(value = {
    ...
    public ResponseEntity<?> getUser(
        ...
        try {
            ...
            if (cause instanceof NoSuchElementException) {
                ...
                logger.error("Execution error retrieving user", e);
                return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(e.getMessage());
            }
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
    ...
    public ResponseEntity<?> listUsers() {
        try {
            ...
            if (cause instanceof IllegalArgumentException) {
                ...
                logger.error("Execution error listing users", e);
                return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(e.getMessage());
            }
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
    ...
    @Operation(summary = "Search Users", description = "Search users by a simple field condition (field, operator, value)")
    @ApiResponse(responseCode = "200", description = "OK", content = @Content(schema = @Schema(implementation = ArrayNode.class)))
    ...
    public ResponseEntity<?> searchUsers(
        ...
        try {
            ...
            if (cause instanceof IllegalArgumentException) {
                ...
                logger.error("Execution error searching users", e);
                return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(e.getMessage());
            }
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
    ...
    static class UserRequest {
        ...
        public String email;
        @Schema(description = "Preferences map")
        ...
        public java.util.Map<String, Object> notificationPreferences;

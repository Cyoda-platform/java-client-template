...
@Component
public class StoreHNItemFailedCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public StoreHNItemFailedCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<ImportJob> context) {
        ...
        try {
            if ("FAILED".equalsIgnoreCase(job.getStatus())) {
                return EvaluationOutcome.fail("job failed", StandardEvalReasonCategories.VALIDATION_FAILURE);
            }
            return EvaluationOutcome.success();
        } catch (Exception e) {
            logger.error("StoreHNItemFailedCriterion error: {}", e.getMessage(), e);

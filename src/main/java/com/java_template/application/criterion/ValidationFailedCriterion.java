...
public class ValidationFailedCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<GetUserJob> context) {
         ...
         // Validate requestUserId presence
         if (entity.getRequestUserId() == null || entity.getRequestUserId().isBlank()) {
            return EvaluationOutcome.fail("User ID is required", StandardEvalReasonCategories.VALIDATION_FAILURE);
         }

         // Validate requestUserId is a positive integer
         try {
             int id = Integer.parseInt(entity.getRequestUserId().trim());
             if (id <= 0) {
                 return EvaluationOutcome.fail("User ID must be a positive integer", StandardEvalReasonCategories.VALIDATION_FAILURE);
             }

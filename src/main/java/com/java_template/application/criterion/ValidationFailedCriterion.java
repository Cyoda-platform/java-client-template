...
public class ValidationFailedCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<ExtractionJob> context) {
        ...
        if (job.getFailureReason() != null && (job.getFailureReason().contains("INVALID") || job.getFailureReason().contains("MISSING"))) {
            return EvaluationOutcome.fail("Validation failed: " + job.getFailureReason(), StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

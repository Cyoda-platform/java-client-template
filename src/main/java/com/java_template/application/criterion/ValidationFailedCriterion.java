...
public class ValidationFailedCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Laureate> context) {
        ...
        Map<String,String> v = l.getValidations();
        if (v == null || v.isEmpty()) {
            return EvaluationOutcome.success();
        }
        // if any critical keys present -> fail
        if (v.containsKey("id") || v.containsKey("name") || v.containsKey("year") || v.containsKey("category")) {
            return EvaluationOutcome.fail("Critical validation failures", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }
        // Only warnings
        return EvaluationOutcome.success();
    }

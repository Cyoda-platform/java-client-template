...
public class ValidationFailedCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Subscriber> context) {
        ...
        // For example, check if the email is already unsubscribed (unsubscribedAt not null)
        if (entity.getUnsubscribedAt() != null) {
            return EvaluationOutcome.fail("Subscriber is unsubscribed", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }

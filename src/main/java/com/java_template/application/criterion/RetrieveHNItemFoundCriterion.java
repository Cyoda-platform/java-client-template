...
@Component
public class RetrieveHNItemFoundCriterion implements CyodaCriterion {
    ...
    public RetrieveHNItemFoundCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<RetrievalJob> context) {
        ...
        try {
            if ("FOUND".equalsIgnoreCase(job.getStatus())) {
                return EvaluationOutcome.success();
            }
            return EvaluationOutcome.fail("item not found", StandardEvalReasonCategories.DATA_QUALITY_FAILURE);
        } catch (Exception e) {
            logger.error("RetrieveHNItemFoundCriterion error: {}", e.getMessage(), e);

...
@Component
public class DeliverySucceededCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public DeliverySucceededCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<DataIngestJob> context) {
        ...
        if ("COMPLETED".equalsIgnoreCase(job.getStatus())) return EvaluationOutcome.success();
        return EvaluationOutcome.fail("Job not completed", StandardEvalReasonCategories.VALIDATION_FAILURE);
    }
}

...
@Component
public class AllDeliveriesSucceededCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public AllDeliveriesSucceededCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<ChangeEvent> context) {
        ...
        for (DeliveryRecord r : recs) {
            if (r.getResult() != DeliveryRecord.Result.SUCCESS) {
                return EvaluationOutcome.fail("Not all deliveries succeeded", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
            }
        }

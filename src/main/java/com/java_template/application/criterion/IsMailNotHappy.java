...
@Component
public class IsMailNotHappy implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public IsMailNotHappy(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Mail> context) {
        ...
        for (String recipient : mail.getMailList()) {
            if (recipient == null || recipient.isBlank()) {
                return EvaluationOutcome.fail("Mail list contains invalid recipient", StandardEvalReasonCategories.DATA_QUALITY_FAILURE);
            }
        }

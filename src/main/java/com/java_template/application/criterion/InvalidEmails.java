...
@Component
public class InvalidEmails implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public InvalidEmails(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Mail> context) {
        ...
        List<String> invalidEmails = mail.getMailList().stream()
            .filter(email -> email == null || email.isBlank() || !email.contains("@"))
            .collect(Collectors.toList());

        if (!invalidEmails.isEmpty()) {
            return EvaluationOutcome.fail("Invalid emails found: " + invalidEmails, StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

...
@Component
public class PurrfectPetsJobStartCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;

    public PurrfectPetsJobStartCriterion(SerializerFactory serializerFactory) {
        this.serializer = serializerFactory.getDefaultCriteriaSerializer();
        logger.info("PurrfectPetsJobStartCriterion initialized with SerializerFactory");
    }
    ...
    public boolean supports(OperationSpecification modelSpec) {
        return "PurrfectPetsJobStartCriterion".equals(modelSpec.operationName()) &&
    ...
    private EvaluationOutcome validateEntity(PurrfectPetsJob entity) {
        // Validate jobId, petType, action, status, createdAt according to business requirements
        ...
        // Status should be PENDING to start processing
        if (entity.getStatus() == null) {
            return EvaluationOutcome.fail("Status is required", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }
        if (entity.getStatus() != PurrfectPetsJob.StatusEnum.PENDING) {

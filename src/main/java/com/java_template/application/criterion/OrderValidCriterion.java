...
public class OrderValidCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Order> context) {
        ...
        // Validation: orderId, petId, status must not be null or blank, quantity must not be null
        ...
        // Additional business rule: quantity must be > 0
        if (order.getQuantity() <= 0) {
            return EvaluationOutcome.fail("quantity must be greater than zero", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }

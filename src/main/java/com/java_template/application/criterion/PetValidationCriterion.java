...
public class PetValidationCriterion implements CyodaCriterion {
    ...
    public EntityCriteriaCalculationResponse check(CyodaEventContext<EntityCriteriaCalculationRequest> context) {
        ...
        return serializer.withRequest(request)
            ...
            .withErrorHandler((error, petJson) -> {
    ...
    private EvaluationOutcome validatePet(Pet pet) {
        if (pet.getName() == null || pet.getName().isBlank()) {
            return EvaluationOutcome.fail("Name must not be blank.", EvaluationOutcome.StandardEvalReasonCategories.VALIDATION_FAILURE);
        }
        if (pet.getType() == null || pet.getType().isBlank()) {
            return EvaluationOutcome.fail("Type must not be blank.", EvaluationOutcome.StandardEvalReasonCategories.VALIDATION_FAILURE);
        }
        if (pet.getStatus() == null || pet.getStatus().isBlank()) {
            return EvaluationOutcome.fail("Status must not be blank.", EvaluationOutcome.StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

...
/**
 * IsAfternoonCriterion - Checks if greeting timestamp is 12:00 or later
 */
@Component
public class IsAfternoonCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public IsAfternoonCriterion(SerializerFactory serializerFactory) {
    ...
    public EntityCriteriaCalculationResponse check(CyodaEventContext<EntityCriteriaCalculationRequest> context) {
        ...
        logger.debug("Checking IsAfternoon criteria for request: {}", request.getId());
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Greeting> context) {
        ...
        if (time.isAfter(LocalTime.of(11, 59))) {
            logger.debug("Greeting is in afternoon (12:00 or later)");
            return EvaluationOutcome.success();
        }

        logger.debug("Greeting is NOT in afternoon (before 12:00)");
        return EvaluationOutcome.fail("Time is before 12:00", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
    }
}

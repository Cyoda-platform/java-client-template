...
@Component
public class LaureateValidationSuccessCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public LaureateValidationSuccessCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Laureate> context) {
        ...
        // Success criterion: borncountrycode should be normalized uppercase
        if (laureate.getBorncountrycode() != null && !laureate.getBorncountrycode().isEmpty()) {
            ...
            if (!code.equals(code.toUpperCase())) {
                return EvaluationOutcome.fail("Borncountrycode must be uppercase", StandardEvalReasonCategories.VALIDATION_FAILURE);
            }
        }

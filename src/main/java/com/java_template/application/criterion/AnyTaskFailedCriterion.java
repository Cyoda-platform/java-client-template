...
@Component
public class AnyTaskFailedCriterion implements CyodaCriterion {
    ...
    public AnyTaskFailedCriterion(SerializerFactory serializerFactory, EntityService entityService) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<ImportJob> context) {
        ...
        try {
            ...
            if (tasks == null || tasks.size() == 0) {
                return EvaluationOutcome.success(); // no tasks -> not failed
            }
            for (com.fasterxml.jackson.databind.JsonNode t : tasks) {
                ...
                if ("FAILED".equalsIgnoreCase(status)) {
                    return EvaluationOutcome.fail("At least one task failed", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
                }
            }
            return EvaluationOutcome.success();
        } catch (Exception e) {
            logger.error("Error evaluating AnyTaskFailedCriterion for job {}: {}", job.getTechnicalId(), e.getMessage(), e);

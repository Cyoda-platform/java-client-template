...
@Component
public class EmailsSentSuccessfully implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public EmailsSentSuccessfully(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<WeeklyCatFactJob> context) {
        ...
        // Business logic: status must be COMPLETED
        if (entity.getStatus() == null || !entity.getStatus().equals("COMPLETED")) {
            return EvaluationOutcome.fail("Emails not sent successfully", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }

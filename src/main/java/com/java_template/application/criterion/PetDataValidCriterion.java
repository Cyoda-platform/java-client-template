...
public class PetDataValidCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(Pet pet) {
        ...
        // Additional example validation: age must be non-negative if present
        if (pet.getAge() != null && pet.getAge() < 0) {
            return EvaluationOutcome.fail("Pet age cannot be negative", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

...
public class PetDataValidCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(Pet entity) {
        if (entity.getPetId() == null || entity.getPetId().isBlank()) {
            return EvaluationOutcome.fail("Pet ID must not be empty", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }
        if (entity.getName() == null || entity.getName().isBlank()) {
            return EvaluationOutcome.fail("Pet name must not be empty", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }
        if (entity.getSpecies() == null || entity.getSpecies().isBlank()) {
            return EvaluationOutcome.fail("Pet species must not be empty", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }
        ...
        if (entity.getStatus() == null || entity.getStatus().isBlank()) {
            return EvaluationOutcome.fail("Pet status must not be empty", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

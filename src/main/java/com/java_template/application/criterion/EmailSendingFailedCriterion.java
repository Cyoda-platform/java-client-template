...
@Component
public class EmailSendingFailedCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public EmailSendingFailedCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<DigestRequest> context) {
        ...
        if (!"FAILED".equalsIgnoreCase(digestRequest.getStatus())) {
            return EvaluationOutcome.fail("Email sending failed criterion mismatch: status is not FAILED", StandardEvalReasonCategories.DATA_QUALITY_FAILURE);
        }

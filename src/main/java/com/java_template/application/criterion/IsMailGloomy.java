...
public class IsMailGloomy implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Mail> context) {
        ...
        if (mail.getIsHappy() == null) {
            return EvaluationOutcome.fail("isHappy field is required to determine gloominess", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }
        ...
        if (mail.getMailType() != null && !"gloomy".equalsIgnoreCase(mail.getMailType())) {
            return EvaluationOutcome.fail("mailType must be 'gloomy' if mail is gloomy", StandardEvalReasonCategories.DATA_QUALITY_FAILURE);
        }

...
public class IsMailGloomy implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Mail> context) {
         ...
         // Validate that the mail is not happy (gloomy)
         if (mail.isHappy()) {
             return EvaluationOutcome.fail("Mail is marked as happy, expected gloomy", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
         }
         ...
         if (mail.getContent() == null || mail.getContent().isBlank()) {
            return EvaluationOutcome.fail("Mail content must not be blank", StandardEvalReasonCategories.VALIDATION_FAILURE);
         }

...
public class ProductPerformanceReportCriterion implements CyodaCriterion {
    ...
    public ProductPerformanceReportCriterion(SerializerFactory serializerFactory) {
    ...
    public EntityCriteriaCalculationResponse check(CyodaEventContext<EntityCriteriaCalculationRequest> context) {
        ...
        logger.debug("Checking ProductPerformanceReport criteria for request: {}", request.getId());

        return serializer.withRequest(request)
                .evaluateEntity(ProductPerformanceReport.class, this::validateEntity)
    ...
    private EvaluationOutcome validateEntity(
            CriterionSerializer.CriterionEntityEvaluationContext<ProductPerformanceReport> context) {
        ProductPerformanceReport entity = context.entityWithMetadata().entity();

        if (entity == null) {
            logger.warn("ProductPerformanceReport is null");
        ...
        if (!entity.isValid(context.entityWithMetadata().metadata())) {
            logger.warn("ProductPerformanceReport is not valid");

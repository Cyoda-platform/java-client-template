...
public class ValidateUserCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<User> context) {
        ...
        if (user == null) {
            return EvaluationOutcome.fail("User missing", StandardEvalReasonCategories.DATA_QUALITY_FAILURE);
        }
        ...
        if (user.getPassword() == null || user.getPassword().length() < 8) {
            return EvaluationOutcome.fail("Password does not meet length policy", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

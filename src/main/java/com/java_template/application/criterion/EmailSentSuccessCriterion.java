...
public class EmailSentSuccessCriterion implements CyodaCriterion {
    ...
    public EmailSentSuccessCriterion(SerializerFactory serializerFactory) {
        ...
        logger.info("EmailSentSuccessCriterion initialized with SerializerFactory");
    }
    ...
    private EvaluationOutcome validateEntity(ProductPerformanceJob entity) {
        if (entity.getStatus() == null || !"COMPLETED".equalsIgnoreCase(entity.getStatus())) {
            return EvaluationOutcome.fail("Email was not sent successfully", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }

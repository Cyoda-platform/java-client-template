...
public class IsPetValidCriterion implements CyodaCriterion {
    ...
    public IsPetValidCriterion(SerializerFactory serializerFactory) {
        this.serializer = serializerFactory.getDefaultCriteriaSerializer();
        logger.info("IsPetValidCriterion initialized with SerializerFactory");
    }
    ...
    public boolean supports(OperationSpecification modelSpec) {
        return "IsPetValidCriterion".equals(modelSpec.operationName()) &&
    ...
    private EvaluationOutcome validateEntity(PetUpdateJob entity) {
        // Validate job status is COMPLETED and jobId, source are present
        ...
        if (entity.getSource() == null || entity.getSource().isBlank()) {
            return EvaluationOutcome.fail("Source is missing", StandardEvalReasonCategories.DATA_QUALITY_FAILURE);
        }

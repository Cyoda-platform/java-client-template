...
@Component
public class CompanyNameInvalidCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public CompanyNameInvalidCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<RetrievalJob> context) {
        ...
        // Validation logic: companyName must be null or blank to be invalid
        if (retrievalJob.getCompanyName() == null || retrievalJob.getCompanyName().isBlank()) {
            return EvaluationOutcome.success();
        }

        return EvaluationOutcome.fail("Company name is valid, expected invalid", StandardEvalReasonCategories.VALIDATION_FAILURE);
    }
}

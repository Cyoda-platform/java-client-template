...
public class PetAvailableCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Order> context) {
        ...
        // Business rule: We consider pet availability as status of the order's pet
        // Since we do not have direct Pet entity here, we assume order.status reflects pet availability status
        // Check if order.status == "AVAILABLE"
        if (order.getStatus() == null || !order.getStatus().equalsIgnoreCase("AVAILABLE")) {

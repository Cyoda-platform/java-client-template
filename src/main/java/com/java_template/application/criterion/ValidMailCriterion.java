...
@Component
public class ValidMailCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public ValidMailCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Mail> context) {
        ...
        for (String email : mail.getMailList()) {
            if (email == null || email.isBlank()) {
                return EvaluationOutcome.fail("Mail list contains empty or null email", StandardEvalReasonCategories.VALIDATION_FAILURE);
            }
        }
        if (mail.getSubject() == null || mail.getSubject().isBlank()) {
            return EvaluationOutcome.fail("Subject must not be empty", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }
        if (mail.getContent() == null || mail.getContent().isBlank()) {
            return EvaluationOutcome.fail("Content must not be empty", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

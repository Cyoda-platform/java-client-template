...
public class SourceUrlInvalidCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(Pet entity) {
        // Validate sourceUrl is null, blank, or invalid format
        if (entity.getPetId() == null || entity.getPetId().isBlank()) {
            return EvaluationOutcome.success(); // If missing, this criterion passes as invalid source
        }
        if (!entity.getPetId().startsWith("http://") && !entity.getPetId().startsWith("https://")) {
            return EvaluationOutcome.success(); // If format invalid, criterion passes
        }
        return EvaluationOutcome.fail("Source URL is valid", StandardEvalReasonCategories.VALIDATION_FAILURE);
    }
}

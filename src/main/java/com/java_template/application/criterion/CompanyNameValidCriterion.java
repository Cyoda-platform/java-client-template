...
@Component
public class CompanyNameValidCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public CompanyNameValidCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<RetrievalJob> context) {
        ...
        // Validation logic: companyName must not be null or blank
        if (retrievalJob.getCompanyName() == null || retrievalJob.getCompanyName().isBlank()) {
            return EvaluationOutcome.fail("Company name is required", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

...
@Component
public class PetNotExistsCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;

    public PetNotExistsCriterion(SerializerFactory serializerFactory) {
        this.serializer = serializerFactory.getDefaultCriteriaSerializer();
        logger.info("PetNotExistsCriterion initialized with SerializerFactory");
    }
    ...
    private EvaluationOutcome validateEntity(AdoptionRequest adoptionRequest) {
        ...
        // Here would be a check if pet does NOT exist in system - assumed to be external or database check
        // For the purpose of this criterion, assume petId presence means pet exists, so fail if present
        // Inverse logic of PetExistsCriterion
        return EvaluationOutcome.fail("Pet does not exist in system", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
    }
}

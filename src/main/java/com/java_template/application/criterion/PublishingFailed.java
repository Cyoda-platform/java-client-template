...
@Component
public class PublishingFailed implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public PublishingFailed(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<WeeklyCatFactJob> context) {
        ...
        // Business logic: status must be FAILED
        if (entity.getStatus() == null || !entity.getStatus().equals("FAILED")) {
            return EvaluationOutcome.fail("Publishing not failed", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }

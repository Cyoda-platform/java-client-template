...
public class PaymentValidCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Payment> context) {
        ...
        if (payment.getProvider() == null || !"DUMMY".equals(payment.getProvider())) {
            return EvaluationOutcome.fail("Only DUMMY provider is supported", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }

...
@Component
public class IsNoSendCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public IsNoSendCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Mail> context) {
        ...
        // Business rule: NoSend criterion applies if mailList is empty or null
        if (mail.getMailList() != null && !mail.getMailList().isEmpty()) {
            return EvaluationOutcome.fail("Mail list is not empty, so NoSend criterion does not apply", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }

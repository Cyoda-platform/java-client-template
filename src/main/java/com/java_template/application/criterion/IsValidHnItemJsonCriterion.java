...
@Component
public class IsValidHnItemJsonCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public IsValidHnItemJsonCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<HackerNewsItem> context) {
        ...
        if (hackerNewsItem.getRawJson() == null || hackerNewsItem.getRawJson().isBlank()) {
            return EvaluationOutcome.fail("Raw JSON must be present", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }
        ...
        if (hackerNewsItem.getBy() == null || hackerNewsItem.getBy().isBlank()) {
            return EvaluationOutcome.fail("Author username must be present", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

        if (hackerNewsItem.getTitle() == null || hackerNewsItem.getTitle().isBlank()) {
            return EvaluationOutcome.fail("Title must be present", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

        if (hackerNewsItem.getType() == null || hackerNewsItem.getType().isBlank()) {
            return EvaluationOutcome.fail("Type must be present", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

...
@Component
public class IsScheduledCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public IsScheduledCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<BatchJob> context) {
        ...
        if (job.getScheduledFor() == null || job.getScheduledFor().isBlank()) {
            return EvaluationOutcome.fail("No scheduledFor provided", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }
        try {
            ...
            if (scheduled.isAfter(Instant.now())) {
                ...
                return EvaluationOutcome.fail("scheduledFor is not in the future", StandardEvalReasonCategories.VALIDATION_FAILURE);
            }

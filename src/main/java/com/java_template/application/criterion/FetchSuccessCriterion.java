...
public class FetchSuccessCriterion implements CyodaCriterion {
    ...
    public FetchSuccessCriterion(CriterionSerializer.SerializerFactory serializerFactory) {
        this.serializer = serializerFactory.getDefaultCriteriaSerializer();
        logger.info("FetchSuccessCriterion initialized with SerializerFactory");
    }
    ...
    private EvaluationOutcome applyValidation(GameScoreFetchJob entity) {
        ...
        if (!"COMPLETED".equalsIgnoreCase(entity.getStatus())) {
            return EvaluationOutcome.fail("Status indicates failure or incomplete", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }

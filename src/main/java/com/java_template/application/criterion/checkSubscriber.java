...
@Component
public class checkSubscriber implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public checkSubscriber(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Subscriber> context) {
         ...
         if (subscriber.getContactValue() == null || subscriber.getContactValue().trim().isEmpty()) {
             return EvaluationOutcome.fail("Contact value is required", StandardEvalReasonCategories.VALIDATION_FAILURE);
         }
         // isActive must not be null
         if (subscriber.getIsActive() == null) {
             return EvaluationOutcome.fail("Active status (isActive) is required", StandardEvalReasonCategories.VALIDATION_FAILURE);
         }

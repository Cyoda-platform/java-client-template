...
public class CartHasItemsCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Cart> context) {
        ...
        if (cart.getTotalItems() == null || cart.getTotalItems() <= 0) {
            return EvaluationOutcome.fail("Cart total items is zero or negative", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

        for (Cart.CartLine line : cart.getLines()) {
            if (line.getQty() == null || line.getQty() <= 0) {
                return EvaluationOutcome.fail("Invalid quantity for item " + line.getSku(), StandardEvalReasonCategories.VALIDATION_FAILURE);
            }
        }

...
@Component
public class ValidPetJobCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;

    public ValidPetJobCriterion(SerializerFactory serializerFactory) {
        this.serializer = serializerFactory.getDefaultCriteriaSerializer();
        logger.info("ValidPetJobCriterion initialized with SerializerFactory");
    }
    ...
    public boolean supports(OperationSpecification modelSpec) {
        return "ValidPetJobCriterion".equals(modelSpec.operationName()) &&
               "pet".equalsIgnoreCase(modelSpec.modelKey().getName()) &&
               Integer.parseInt(Config.ENTITY_VERSION) == modelSpec.modelKey().getVersion();
    }

    private EvaluationOutcome validateEntity(Pet entity) {
        // Validate that the pet has all required fields filled and status is ACTIVE
        ...
        if (!entity.getStatus().equalsIgnoreCase("ACTIVE")) {
            return EvaluationOutcome.fail("status must be ACTIVE", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }

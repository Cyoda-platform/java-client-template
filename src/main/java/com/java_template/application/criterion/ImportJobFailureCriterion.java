...
@Component
public class ImportJobFailureCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public ImportJobFailureCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<ImportJob> context) {
        ...
        // For simplicity: if status explicitly set to FAILED, consider failure
        if ("FAILED".equalsIgnoreCase(job.getStatus())) {
            return EvaluationOutcome.success();
        }
        return EvaluationOutcome.fail("Import job not failed", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
    }
}

...
public class IngestionFailureCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Job> context) {
         ...
         // Validation logic: The ingestion failure is considered valid if job status is "FAILED" and errorMessage is present
         if (job.getStatus() == null || !"FAILED".equals(job.getStatus())) {
         ...
         if (job.getErrorMessage() == null || job.getErrorMessage().isBlank()) {
             return EvaluationOutcome.fail("Error message is required for a failed job", StandardEvalReasonCategories.DATA_QUALITY_FAILURE);
         }

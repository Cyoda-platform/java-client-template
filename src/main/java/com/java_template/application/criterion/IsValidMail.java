...
@Component
public class IsValidMail implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public IsValidMail(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Mail> context) {
        ...
        if (mail.getMailList() == null || mail.getMailList().isEmpty()) {
            return EvaluationOutcome.fail("mailList cannot be null or empty", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

        if (mail.getSubject() == null || mail.getSubject().isBlank()) {
            return EvaluationOutcome.fail("subject cannot be null or blank", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

        if (mail.getContent() == null || mail.getContent().isBlank()) {
            return EvaluationOutcome.fail("content cannot be null or blank", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

...
public class SubscriberEmailValidCriterion implements CyodaCriterion {
    ...
    public SubscriberEmailValidCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<WeeklyCatFactJob> context) {
        ...
        // Check if subscriber already exists (simulate with method stub)
        if (!checkSubscriberExists(email)) {
            return EvaluationOutcome.fail("Subscriber does not exist", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }
    ...
    private boolean checkSubscriberExists(String email) {
        // Placeholder for actual existence check logic, e.g., DB lookup
        // For prototype, assume subscriber exists if email ends with '@example.com'

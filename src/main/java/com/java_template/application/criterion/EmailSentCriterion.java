...
public class EmailSentCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Report> context) {
        ...
        if (r == null) return EvaluationOutcome.fail("Report missing", StandardEvalReasonCategories.VALIDATION_FAILURE);
        if (r.getStatus() != null && r.getStatus().equals("SENT")) return EvaluationOutcome.success();
        return EvaluationOutcome.fail("Report not sent", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
    }
}

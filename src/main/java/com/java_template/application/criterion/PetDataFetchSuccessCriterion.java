...
@Component
public class PetDataFetchSuccessCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;

    public PetDataFetchSuccessCriterion(SerializerFactory serializerFactory) {
        this.serializer = serializerFactory.getDefaultCriteriaSerializer();
        logger.info("PetDataFetchSuccessCriterion initialized with SerializerFactory");
    }
    ...
    public boolean supports(OperationSpecification modelSpec) {
        return "PetDataFetchSuccessCriterion".equals(modelSpec.operationName()) &&
    ...
    private EvaluationOutcome validateEntity(Pet entity) {
        if (entity.getPetId() == null) {
            return EvaluationOutcome.fail("Pet ID must be present", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

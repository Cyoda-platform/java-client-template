...
@Component
public class ReportIncompleteCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public ReportIncompleteCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<AnalysisReport> context) {
        ...
        if ("FAILED".equalsIgnoreCase(report.getStatus())) return EvaluationOutcome.success();
        return EvaluationOutcome.fail("Report not marked FAILED", StandardEvalReasonCategories.VALIDATION_FAILURE);
    }
}

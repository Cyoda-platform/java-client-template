...
@Component
public class NotificationFailedCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public NotificationFailedCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<ExtractionJob> context) {
        ...
        // If failureReason indicates notification failure
        if (job.getFailureReason() != null && job.getFailureReason().contains("EMAIL")) {
            return EvaluationOutcome.fail("Notification failure recorded: " + job.getFailureReason(), StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }

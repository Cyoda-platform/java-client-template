...
@Component
public class IsJobInvalidCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;

    public IsJobInvalidCriterion(SerializerFactory serializerFactory) {
        this.serializer = serializerFactory.getDefaultCriteriaSerializer();
        logger.info("IsJobInvalidCriterion initialized with SerializerFactory");
    }
    ...
    public boolean supports(OperationSpecification modelSpec) {
        return "IsJobInvalidCriterion".equals(modelSpec.operationName()) &&
    ...
    private EvaluationOutcome validateEntity(PetRegistrationJob entity) {
        // Business logic: job must be invalid (fails isValid())
        if (entity.isValid()) {

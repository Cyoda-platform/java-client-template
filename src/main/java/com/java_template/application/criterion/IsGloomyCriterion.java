...
public class IsGloomyCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Mail> context) {
        ...
        // If any recipient explicitly indicates gloomy, classify as gloomy
        for (String recipient : list) {
            if (recipient != null && recipient.equalsIgnoreCase("gloom@example.com")) {
                return EvaluationOutcome.success();
            }
        }

        // Otherwise not gloomy
        return EvaluationOutcome.fail("No gloomy indicators found", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
    }
}

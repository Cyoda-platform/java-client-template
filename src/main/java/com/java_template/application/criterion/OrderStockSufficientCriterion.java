...
@Component
public class OrderStockSufficientCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public OrderStockSufficientCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Order> context) {
        ...
        // Prototype: unitPrice >= 0 indicates sufficient stock
        for (OrderItem item : order.getItems()) {
            if (item == null) continue;
            if (item.getUnitPrice() == null || item.getUnitPrice().compareTo(java.math.BigDecimal.ZERO) < 0) {
                return EvaluationOutcome.fail("Item indicates insufficient stock", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
            }
        }

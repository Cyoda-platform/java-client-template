...
@Component
public class IsMailInvalid implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public IsMailInvalid(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Mail> context) {
        ...
        for (String email : mail.getMailList()) {
            if (email == null || email.isBlank()) {
                return EvaluationOutcome.fail("Mail list contains blank or null email", StandardEvalReasonCategories.DATA_QUALITY_FAILURE);
            }
        }
        if (mail.getIsHappy() != null) {
            return EvaluationOutcome.fail("isHappy flag should not be set for invalid mail", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

...
@Component
public class SomeDeliveriesFailedButRetryableCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public SomeDeliveriesFailedButRetryableCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<ChangeEvent> context) {
        ...
        for (DeliveryRecord r : recs) {
            if (r.getResult() == DeliveryRecord.Result.SUCCESS) continue;
            if (r.getResult() == DeliveryRecord.Result.RETRY_SCHEDULED) {
                hasRetryable = true;
            } else if (r.getResult() == DeliveryRecord.Result.FAILED) {
                hasFailed = true;
            }
        }
        if (hasRetryable) {
        ...
        if (hasFailed && !hasRetryable) {
            return EvaluationOutcome.fail("All failed and not retryable", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }
        return EvaluationOutcome.fail("No retryable failures detected", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
    }
}

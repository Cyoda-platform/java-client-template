...
@Component
public class NoAnomalyCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public NoAnomalyCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Activity> context) {
        ...
        if (Boolean.TRUE.equals(activity.getAnomalyFlag())) {
            return EvaluationOutcome.fail("anomaly present", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }

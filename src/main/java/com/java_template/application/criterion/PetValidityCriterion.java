...
public class PetValidityCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(Pet pet) {
        ...
        if (pet.getPetId() == null || pet.getPetId().isBlank()) {
            return EvaluationOutcome.fail("Pet petId is required", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }
        ...
        if (pet.getLifecycleStatus() == null) {
            return EvaluationOutcome.fail("Pet lifecycleStatus is required", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }
        if (pet.getCreatedAt() == null) {
            return EvaluationOutcome.fail("Pet createdAt timestamp is required", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

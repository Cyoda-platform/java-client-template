...
@Component
public class IsReportCompleteCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public IsReportCompleteCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<MonthlyReport> context) {
        ...
        if (report.getTotalUsers() == null || report.getGeneratedAt() == null) {
            return EvaluationOutcome.fail("Report metrics incomplete", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

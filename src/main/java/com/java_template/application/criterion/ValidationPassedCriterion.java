...
public class ValidationPassedCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<GetUserJob> context) {
         ...
         if (requestUserId == null || requestUserId.isBlank()) {
             return EvaluationOutcome.fail("requestUserId is required", StandardEvalReasonCategories.VALIDATION_FAILURE);
         }
         ...
         try {
             ...
             if (id <= 0) {
                 return EvaluationOutcome.fail("requestUserId must be a positive integer", StandardEvalReasonCategories.VALIDATION_FAILURE);
             }

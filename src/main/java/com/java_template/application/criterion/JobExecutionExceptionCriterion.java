...
public class JobExecutionExceptionCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Job> context) {
         ...
         if (attempts < MAX_ATTEMPTS) {
             logger.info("Job failed but eligible for retry (attempts={} < max={}) for job type={}", attempts, MAX_ATTEMPTS, job.getType());
             return EvaluationOutcome.fail("Job failed and is eligible for retry", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
         } else {
             logger.info("Job failed and reached max attempts (attempts={} >= max={}) - escalate to ADMIN for job type={}", attempts, MAX_ATTEMPTS, job.getType());
             return EvaluationOutcome.fail("Job failed and reached max retry attempts; escalate to ADMIN", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
         }
    }
}
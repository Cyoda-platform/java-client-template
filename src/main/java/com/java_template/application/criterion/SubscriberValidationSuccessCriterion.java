...
public class SubscriberValidationSuccessCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Subscriber> context) {
        ...
        if (subscriber.getContactType() != null && !subscriber.getContactType().isEmpty()
                && subscriber.getContactAddress() != null && !subscriber.getContactAddress().isEmpty()) {
            return EvaluationOutcome.success();
        }
        return EvaluationOutcome.fail("Contact type and address must be present", StandardEvalReasonCategories.VALIDATION_FAILURE);
    }
}

...
@Component
public class IsSuccessCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;

    public IsSuccessCriterion(SerializerFactory serializerFactory) {
        this.serializer = serializerFactory.getDefaultCriteriaSerializer();
        logger.info("IsSuccessCriterion initialized with SerializerFactory");
    }
    ...
    public boolean supports(OperationSpecification modelSpec) {
        return "IsSuccessCriterion".equals(modelSpec.operationName()) &&
    ...
    private EvaluationOutcome validateEntity(CatFactJob entity) {
        ...
        if (!"COMPLETED".equals(entity.getStatus())) {
            return EvaluationOutcome.fail("Status must be COMPLETED", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }
        if (entity.getCatFactText() == null || entity.getCatFactText().isBlank()) {
            return EvaluationOutcome.fail("CatFactText must not be null or blank", StandardEvalReasonCategories.DATA_QUALITY_FAILURE);
        }

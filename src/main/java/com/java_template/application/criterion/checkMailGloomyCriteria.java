...
@Component
public class checkMailGloomyCriteria implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public checkMailGloomyCriteria(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Mail> context) {
        ...
        // Validation logic for Gloomy Mail
        ...
        for (String email : mail.getMailList()) {
            if (email == null || email.isBlank()) {
                return EvaluationOutcome.fail("Mail list contains invalid email", StandardEvalReasonCategories.VALIDATION_FAILURE);
            }
        }

        if (mail.getSubject() == null || mail.getSubject().isBlank()) {
            return EvaluationOutcome.fail("Subject must not be blank", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

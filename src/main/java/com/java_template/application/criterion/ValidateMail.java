...
@Component
public class ValidateMail implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public ValidateMail(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Mail> context) {
        ...
        // Additional validation: check for valid email format in mailList
        for (String email : mail.getMailList()) {
            if (email == null || !email.matches("^[^@\s]+@[^@\s]+\\.[^@\s]+$")) {
                return EvaluationOutcome.fail("Invalid email format in mailList: " + email, StandardEvalReasonCategories.DATA_QUALITY_FAILURE);
            }
        }

...
@Component
public class IsValidAndNotHappy implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public IsValidAndNotHappy(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Mail> context) {
        ...
        // Validate isHappy must be false
        if (mail.getIsHappy() == null || mail.getIsHappy()) {
            return EvaluationOutcome.fail("Mail must not be gloomy (isHappy=false)", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }

        // Validate content can be null or non-null, no failure

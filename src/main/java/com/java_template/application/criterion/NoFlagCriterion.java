...
@Component
public class NoFlagCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public NoFlagCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Product> context) {
        ...
        if (p.getFlags() != null && !p.getFlags().isEmpty()) {
            return EvaluationOutcome.fail("Product has flags", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }

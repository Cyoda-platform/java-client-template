...
public class RequestValidCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<AdoptionRequest> context) {
        ...
        if (req.getPetTechnicalId() == null || req.getPetTechnicalId().trim().isEmpty()) {
            return EvaluationOutcome.fail("Missing petTechnicalId", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }
        if (req.getUserTechnicalId() == null || req.getUserTechnicalId().trim().isEmpty()) {
            return EvaluationOutcome.fail("Missing userTechnicalId", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

        // Basic pass - deeper checks (pet availability / user status) are handled by processors that can load related entities

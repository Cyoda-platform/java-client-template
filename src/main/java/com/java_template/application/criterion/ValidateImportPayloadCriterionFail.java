...
@Component
public class ValidateImportPayloadCriterionFail implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public ValidateImportPayloadCriterionFail(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<ImportJob> context) {
        ...
        if ("FAILED".equalsIgnoreCase(job.getStatus())) {
            return EvaluationOutcome.fail("import job failed due validation", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

...
public class IsProcessingFailedCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(Pet entity) {
        // Business rule: Pet must have status FAILED to be considered failed processing
        if (entity.getStatus() == null || !entity.getStatus().equalsIgnoreCase("FAILED")) {
            return EvaluationOutcome.fail("Pet status must be FAILED for failed processing", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }

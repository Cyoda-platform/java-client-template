...
public class IsProcessingFailedCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(CommentIngestionJob entity) {
        // Valid if status is FAILED
        if (!"FAILED".equals(entity.getStatus())) {
            return EvaluationOutcome.fail("Status must be FAILED to mark processing failed", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }

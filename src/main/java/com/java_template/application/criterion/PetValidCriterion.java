...
public class PetValidCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(PetJob entity) {
        // Validation: jobId and petType must not be null or blank
        ...
        // Validate status must be PENDING or PROCESSING for a valid pet job
        ...
        if (!status.equals("PENDING") && !status.equals("PROCESSING") && !status.equals("COMPLETED")) {
            return EvaluationOutcome.fail("Invalid status for valid PetJob", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }

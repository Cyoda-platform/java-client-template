...
public class PetValidCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(Pet pet) {
        // Pet must have non-null and non-blank petId, name, category, and status
        ...
        // Additional business rule: status must be one of AVAILABLE, PENDING, SOLD
        String status = pet.getStatus();
        if (!status.equals("AVAILABLE") && !status.equals("PENDING") && !status.equals("SOLD")) {
            return EvaluationOutcome.fail("status must be AVAILABLE, PENDING, or SOLD", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }

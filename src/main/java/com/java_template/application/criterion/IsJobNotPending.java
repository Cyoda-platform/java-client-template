...
@Component
public class IsJobNotPending implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public IsJobNotPending(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<WeeklyCatFactJob> context) {
        ...
        // Business logic: status must not be "PENDING" (null or other statuses)
        if (entity.getStatus() != null && entity.getStatus().equals("PENDING")) {
            return EvaluationOutcome.fail("Status is PENDING", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }

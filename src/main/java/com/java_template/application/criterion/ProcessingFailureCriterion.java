...
public class ProcessingFailureCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<WeeklyCatFactJob> context) {
        ...
        // Validate processing failure: status must be FAILED
        if (entity.getStatus() == null || !entity.getStatus().equalsIgnoreCase("FAILED")) {
            return EvaluationOutcome.fail("Processing is not failed", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }

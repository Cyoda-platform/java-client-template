...
@Component
public class CartItemsReadyForCheckoutCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public CartItemsReadyForCheckoutCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<ShoppingCart> context) {
        ...
        for (ShoppingCartItem item : cart.getItems()) {
            ...
            if (item.getQuantity() == null || item.getQuantity() < 1) {
                return EvaluationOutcome.fail("Item quantity must be >= 1", StandardEvalReasonCategories.VALIDATION_FAILURE);
            }
            if (item.getSku() == null || item.getSku().trim().isEmpty()) {
                return EvaluationOutcome.fail("Item SKU missing", StandardEvalReasonCategories.DATA_QUALITY_FAILURE);
            }
        }

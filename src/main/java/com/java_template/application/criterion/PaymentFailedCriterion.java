...
public class PaymentFailedCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Order> context) {
        ...
        if (order.getPaymentReference() == null || order.getPaymentReference().isBlank()) return EvaluationOutcome.fail("paymentReference missing", StandardEvalReasonCategories.VALIDATION_FAILURE);
        // We cannot inspect provider status here, processors will handle callbacks

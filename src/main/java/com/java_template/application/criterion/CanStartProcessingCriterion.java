...
public class CanStartProcessingCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(Pet entity) {
        // Business rule: Pet must have status PENDING to start processing
        if (entity.getStatus() == null || !entity.getStatus().equalsIgnoreCase("PENDING")) {
            return EvaluationOutcome.fail("Pet status must be PENDING to start processing", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }

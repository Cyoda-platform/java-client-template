...
public class CanStartProcessingCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(EmailDispatch entity) {
        // Business logic: allow processing to start only if status is PENDING and digestRequestId is present
        if (entity.getDigestRequestId() == null || entity.getDigestRequestId().isBlank()) {
            return EvaluationOutcome.fail("Missing digestRequestId", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

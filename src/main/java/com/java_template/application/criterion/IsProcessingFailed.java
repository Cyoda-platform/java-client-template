...
public class IsProcessingFailed implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Workflow> context) {
        ...
        if (!workflow.getStatus().equalsIgnoreCase("FAILED")) {
            return EvaluationOutcome.fail("Workflow status must be FAILED", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }

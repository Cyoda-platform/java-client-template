...
@Component
public class WorkflowSuccessCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public WorkflowSuccessCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Workflow> context) {
         ...
         // Business logic: Validate that workflow status is COMPLETED for success
         if (workflow.getStatus() == null || !workflow.getStatus().equalsIgnoreCase("COMPLETED")) {
            return EvaluationOutcome.fail("Workflow status is not COMPLETED", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
         }
         // Optionally validate name and description are not blank for a successful workflow

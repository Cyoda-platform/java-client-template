...
public class ProcessingSuccessCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<WeeklyCatFactJob> context) {
        ...
        // Validate processing success: status must be COMPLETED
        if (entity.getStatus() == null || !entity.getStatus().equalsIgnoreCase("COMPLETED")) {
            return EvaluationOutcome.fail("Processing is not completed", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }

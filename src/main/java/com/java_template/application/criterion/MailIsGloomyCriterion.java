...
/**
 * MailIsGloomyCriterion - Determines if a mail entity should be processed as gloomy mail
 * 
 * This criterion evaluates the isHappy attribute to determine if the mail should
 * be routed to gloomy mail processing. Returns true only when isHappy is explicitly
 * set to false and the entity is in a valid state.
 */
@Component
public class MailIsGloomyCriterion implements CyodaCriterion {
    ...
    public EntityCriteriaCalculationResponse check(CyodaEventContext<EntityCriteriaCalculationRequest> context) {
        ...
        logger.debug("Checking MailIsGloomyCriterion for request: {}", request.getId());
     ...
     * - isHappy is explicitly set to false
     */
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Mail> context) {
        ...
        if (!"pending".equalsIgnoreCase(currentState)) {
            logger.debug("Mail entity {} is not in PENDING state (current: {}), skipping gloomy criterion", entityId, currentState);
        ...
        // Main evaluation logic - check if mail is gloomy
        boolean isGloomy = Boolean.FALSE.equals(entity.getIsHappy());
        
        logger.debug("MailIsGloomyCriterion evaluation for entity {}: {}", entityId, isGloomy);
        
        if (isGloomy) {
            return EvaluationOutcome.success();
        } else {
            return EvaluationOutcome.fail("Mail is not gloomy (isHappy = true)", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }
    }
}

...
@Component
public class CartItemsRequireAdjustmentCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public CartItemsRequireAdjustmentCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<ShoppingCart> context) {
        ...
        // For prototype: require adjustment if any item quantity < 1
        for (ShoppingCartItem item : cart.getItems()) {
            if (item == null) continue;
            if (item.getQuantity() == null || item.getQuantity() < 1) {
                return EvaluationOutcome.success(); // indicates needs adjustment (used to transition back to Open)
            }
        }
        return EvaluationOutcome.fail("No adjustments required", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
    }
}
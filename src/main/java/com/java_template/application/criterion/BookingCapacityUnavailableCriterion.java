...
@Component
public class BookingCapacityUnavailableCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public BookingCapacityUnavailableCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Booking> context) {
        ...
        // Business logic:
        // Check that event capacity is NOT sufficient for the number of tickets in the booking
        ...
        if (booking.getNumberOfTickets() <= capacity) {
            return EvaluationOutcome.fail("Capacity is available, cannot move to waitlist", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

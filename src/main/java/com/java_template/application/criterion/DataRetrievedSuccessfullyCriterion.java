...
@Component
public class DataRetrievedSuccessfullyCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public DataRetrievedSuccessfullyCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<DigestRequest> context) {
        ...
        if (!"COMPLETED".equalsIgnoreCase(digestRequest.getStatus())) {
            return EvaluationOutcome.fail("Data not retrieved successfully: status is not COMPLETED", StandardEvalReasonCategories.DATA_QUALITY_FAILURE);
        }

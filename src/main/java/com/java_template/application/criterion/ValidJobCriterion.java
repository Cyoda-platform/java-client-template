...
@Component
public class ValidJobCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;

    public ValidJobCriterion(SerializerFactory serializerFactory) {
        this.serializer = serializerFactory.getDefaultCriteriaSerializer();
        logger.info("ValidJobCriterion initialized with SerializerFactory");
    }
    ...
    public boolean supports(OperationSpecification modelSpec) {
        return "ValidJobCriterion".equals(modelSpec.operationName()) &&
    ...
    private EvaluationOutcome validateEntity(CommentIngestionJob entity) {
        ...
        if (entity.getPostId() == null || entity.getPostId() <= 0) {
            return EvaluationOutcome.fail("Post ID must be a positive number", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }
        if (entity.getReportEmail() == null || entity.getReportEmail().isBlank()) {
            return EvaluationOutcome.fail("Report email must be provided", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

...
public class PetJobValidationCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(PetJob entity) {
        // Business logic validation: jobId and status must be non-null and non-blank
        if (entity.getJobId() == null || entity.getJobId().isBlank()) {
            return EvaluationOutcome.fail("Job ID must be provided", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }
        if (entity.getStatus() == null || entity.getStatus().isBlank()) {
            return EvaluationOutcome.fail("Status must be provided", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }
        // Status must be one of the allowed JobStatusEnum values: PENDING, PROCESSING, COMPLETED, FAILED

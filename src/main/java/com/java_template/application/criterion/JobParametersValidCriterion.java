...
public class JobParametersValidCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Job> context) {
        ...
        if (params == null || params.isEmpty()) {
            return EvaluationOutcome.success(); // Parameters are optional
        }
        try {
            new com.fasterxml.jackson.databind.ObjectMapper().readTree(params);
        } catch (Exception e) {
            logger.error("Invalid JSON in job parameters: {}", e.getMessage());
            return EvaluationOutcome.fail("Invalid JSON in job parameters", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

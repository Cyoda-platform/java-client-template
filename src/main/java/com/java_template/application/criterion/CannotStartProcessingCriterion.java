...
@Component
public class CannotStartProcessingCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public CannotStartProcessingCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<HackerNewsItemJob> context) {
        ...
        // Business logic: cannot start processing if status is PENDING
        if (entity.getStatus() == null || entity.getStatus().isBlank()) {
            return EvaluationOutcome.fail("Status is missing", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }
        if ("PENDING".equals(entity.getStatus())) {
            return EvaluationOutcome.fail("Cannot start processing because status is PENDING", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }

...
public class IsProcessingSuccessfulCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(Pet entity) {
        // Business rule: Pet must have status COMPLETED to be considered successful processing
        if (entity.getStatus() == null || !entity.getStatus().equalsIgnoreCase("COMPLETED")) {
            return EvaluationOutcome.fail("Pet status must be COMPLETED for successful processing", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }

...
@Component
public class FormatReportErrorCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public FormatReportErrorCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<InventoryReportJob> context) {
        ...
        // If job status is FAILED or metadata indicates an error, signal failure
        if ("FAILED".equals(job.getStatus())) {
            return EvaluationOutcome.fail("Job is in failed state", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }

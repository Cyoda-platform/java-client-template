...
public class LaureateValidationFailureCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Laureate> context) {
        ...
        // Failure criterion: mandatory fields missing
        ...
        try {
            if (laureate.getBorn() != null && !laureate.getBorn().isEmpty()) {
                java.time.LocalDate.parse(laureate.getBorn());
            }
            if (laureate.getDied() != null && !laureate.getDied().isEmpty()) {
                java.time.LocalDate.parse(laureate.getDied());
            }
        } catch (Exception e) {
            return EvaluationOutcome.fail("Born or died date must be ISO 8601 format", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

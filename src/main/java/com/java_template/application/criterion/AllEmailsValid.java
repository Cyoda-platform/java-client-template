...
@Component
public class AllEmailsValid implements CyodaCriterion {
    ...
    public AllEmailsValid(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Mail> context) {
        ...
        boolean allValid = mail.getMailList().stream().allMatch(email -> email != null && EMAIL_PATTERN.matcher(email).matches());

        if (!allValid) {
            return EvaluationOutcome.fail("One or more emails are invalid", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

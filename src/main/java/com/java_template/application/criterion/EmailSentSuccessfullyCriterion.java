...
@Component
public class EmailSentSuccessfullyCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public EmailSentSuccessfullyCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<DigestRequest> context) {
        ...
        if (!"SENT".equalsIgnoreCase(digestRequest.getStatus())) {
            return EvaluationOutcome.fail("Email not sent successfully: status is not SENT", StandardEvalReasonCategories.DATA_QUALITY_FAILURE);
        }

...
@Component
public class isOrderValid implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public isOrderValid(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Order> context) {
        ...
        // Additional business logic: order status must be 'PLACED' to be valid for this criterion
        if (!"PLACED".equalsIgnoreCase(order.getStatus())) {
            return EvaluationOutcome.fail("Order status must be 'PLACED' for validation", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }

...
public class PersistFailureCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<HackerNewsItem> context) {
        ...
        if (entity == null) {
            return EvaluationOutcome.fail("no entity", StandardEvalReasonCategories.DATA_QUALITY_FAILURE);
        }
        if ("FAILED".equalsIgnoreCase(entity.getStatus())) {
            return EvaluationOutcome.fail("persistence-failed", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }

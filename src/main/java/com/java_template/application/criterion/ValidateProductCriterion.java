...
@Component
public class ValidateProductCriterion implements CyodaCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public ValidateProductCriterion(SerializerFactory serializerFactory) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Product> context) {
        ...
        if (product == null) {
            return EvaluationOutcome.fail("Product missing", StandardEvalReasonCategories.DATA_QUALITY_FAILURE);
        }
        ...
        if (product.getAvailableQuantity() == null || product.getAvailableQuantity() < 0) {
            return EvaluationOutcome.fail("availableQuantity must be >= 0", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

...
public class IsJobPendingCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(ScoreFetchJob entity) {
        // Validation logic for checking if the ScoreFetchJob is pending
        if (entity.getStatus() == null) {
            return EvaluationOutcome.fail("Status is null", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }
        if (!"PENDING".equalsIgnoreCase(entity.getStatus().name())) {
            return EvaluationOutcome.fail("Job status is not PENDING", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        }
        if (entity.getJobDate() == null) {
            return EvaluationOutcome.fail("Job date is required", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }

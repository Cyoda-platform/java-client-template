...
public class ValidateMailCriterion implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(Mail mail) {
        if (mail.getMailList() == null || mail.getMailList().isEmpty()) {
            return EvaluationOutcome.fail("Mail list must not be null or empty", StandardEvalReasonCategories.VALIDATION_FAILURE);
        }
        for (String email : mail.getMailList()) {
            if (email == null || email.isBlank()) {
                return EvaluationOutcome.fail("Mail list contains blank or null email", StandardEvalReasonCategories.VALIDATION_FAILURE);
            }
        }

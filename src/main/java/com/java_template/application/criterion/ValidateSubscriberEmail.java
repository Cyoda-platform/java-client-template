...
public class ValidateSubscriberEmail implements CyodaCriterion {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<Subscriber> context) {
         ...
         // Validation: check preferredPetTypes is not null or empty
         if (subscriber.getPreferredPetTypes() == null || subscriber.getPreferredPetTypes().isEmpty()) {
             return EvaluationOutcome.fail("Preferred pet types must be specified", StandardEvalReasonCategories.VALIDATION_FAILURE);
         }

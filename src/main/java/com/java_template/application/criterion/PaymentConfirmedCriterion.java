...
public class PaymentConfirmedCriterion implements CyodaCriterion {
    ...
    public PaymentConfirmedCriterion(SerializerFactory serializerFactory, ObjectMapper objectMapper) {
    ...
    private EvaluationOutcome validateEntity(CriterionSerializer.CriterionEntityEvaluationContext<CartOrder> context) {
        try {
            // The payment confirmation is represented in the incoming request payload. We need to check if the event indicates success.
            ...
            if ("PaymentResult".equalsIgnoreCase(type) && "Success".equalsIgnoreCase(status)) {
                return EvaluationOutcome.success();
            }
            return EvaluationOutcome.fail("Payment not confirmed", StandardEvalReasonCategories.BUSINESS_RULE_FAILURE);
        } catch (Exception e) {
            logger.error("Error evaluating payment confirmation: {}", e.getMessage(), e);

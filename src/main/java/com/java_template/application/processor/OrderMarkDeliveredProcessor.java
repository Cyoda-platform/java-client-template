...
public class OrderMarkDeliveredProcessor implements CyodaProcessor {
    ...
    private Order processEntityLogic(ProcessorSerializer.ProcessorEntityExecutionContext<Order> context) {
        ...
        // Find associated shipment and update to DELIVERED state
        updateShipmentState(order.getOrderId(), "DELIVER");
    ...
    /**
     * Update the associated shipment state to DELIVERED.
     */
    private void updateShipmentState(String orderId, String transition) {
        try {
            logger.info("Updating shipment to delivered for order: {}", orderId);
            ...
            if (shipmentResponse.isPresent()) {
                ...
                // Update shipment with transition (this will move it to DELIVERED state)
                entityService.update(entityId, shipment, transition);
                
                logger.info("Shipment updated to delivered: {} for order: {}", 

...
public class ShipmentReadyToSendProcessor implements CyodaProcessor {
    ...
    private Shipment processEntityLogic(ProcessorSerializer.ProcessorEntityExecutionContext<Shipment> context) {
        ...
        try {
            // Update qtyPicked for all lines to match qtyOrdered
            ...
            logger.info("Shipment {} ready to send - all items picked", shipment.getShipmentId());

            return shipment;

        } catch (Exception e) {
            logger.error("Error processing shipment ready to send: {}", e.getMessage(), e);
            throw new RuntimeException("Failed to process shipment ready to send: " + e.getMessage(), e);
        }
    }
}
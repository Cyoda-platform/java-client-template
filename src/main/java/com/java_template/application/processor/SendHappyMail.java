...
public class SendHappyMail implements CyodaProcessor {
    ...
    public EntityProcessorCalculationResponse process(CyodaEventContext<EntityProcessorCalculationRequest> context) {
        ...
        logger.info("Processing Mail entity for sending happy mail, request: {}", request.getId());
    ...
    private Mail processEntityLogic(ProcessorSerializer.ProcessorEntityExecutionContext<Mail> context) {
        ...
        // Business logic for sending happy mail
        if (entity.getMailList() == null || entity.getMailList().isEmpty()) {
            logger.warn("Mail list is empty, no mails to send.");
            return entity;
        }
        // Simulate sending mails
        entity.getMailList().forEach(mail -> logger.info("Sending happy mail to: {}", mail));

...
public class PetApprovalProcessor implements CyodaProcessor {
    ...
    private boolean isValidPet(Pet pet) {
        return pet != null && pet.getId() != null && !pet.getName().isEmpty();
    }

    private ErrorInfo handlePetError(Throwable t, Pet pet) {
        logger.error("Error processing Pet: {}", t.getMessage(), t);
        return new ErrorInfo(t.getMessage(), "PET_PROCESSING_ERROR");
    }
    ...
    private boolean businessValidation(Pet pet) {
        return "approved".equals(pet.getStatus());
    }
}

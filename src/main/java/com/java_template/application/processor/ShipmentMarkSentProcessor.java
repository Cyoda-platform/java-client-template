...
public class ShipmentMarkSentProcessor implements CyodaProcessor {
    ...
    private Shipment processEntityLogic(ProcessorSerializer.ProcessorEntityExecutionContext<Shipment> context) {
        ...
        try {
            // Update qtyShipped for all lines to match qtyPicked
            ...
            logger.error("Error processing shipment mark sent: {}", e.getMessage(), e);
            throw new RuntimeException("Failed to process shipment mark sent: " + e.getMessage(), e);
        }
    }
}
 ...
 * 
 * This processor simulates payment processing by waiting approximately 3 seconds
 * and then marking the payment as PAID for demo purposes.
 */
@Component
public class AutoMarkPaidAfter3s implements CyodaProcessor {
    ...
    private EntityWithMetadata<Payment> processAutoPayment(
        ...
        logger.debug("Auto-processing payment: {}", payment.getPaymentId());
        ...
        try {
            ...
            logger.info("Payment {} automatically marked as PAID after 3 seconds", payment.getPaymentId());

...
public class PetProcessor implements CyodaProcessor {
    ...
    public EntityProcessorCalculationResponse process(CyodaEventContext<EntityProcessorCalculationRequest> context) {
        ...
        return serializer.withRequest(request)
                .toEntity(Pet.class)
                .validate(this::validatePet, "Invalid Pet state")
    ...
    private Pet processPetLogic(Pet pet) {
        logger.info("Pet processed and available with ID: {}", pet.getPetId());
        // Additional Pet processing logic can be added here
        return pet;
    }
}

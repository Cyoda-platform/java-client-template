...
public class PetProcessor implements CyodaProcessor {
    ...
    private boolean isValidEntity(Pet pet) {
        return pet != null && pet.getId() != null && !pet.getId().isBlank() &&
                pet.getTechnicalId() != null && pet.getName() != null && !pet.getName().isBlank() &&
                pet.getType() != null && !pet.getType().isBlank() && pet.getStatus() != null && !pet.getStatus().isBlank();
    }

    // Copied logic from CyodaEntityControllerPrototype.processPet (currently only logging, no complex logic)

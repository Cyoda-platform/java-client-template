...
public class PetProcessor implements CyodaProcessor {
    ...
    private Pet processEntityLogic(Pet pet) {
        logger.info("Processing Pet with technicalId: {}", pet.getId());

        if (pet.getName() == null || pet.getName().isBlank() ||
            pet.getSpecies() == null || pet.getSpecies().isBlank()) {
            logger.error("Pet validation failed: name or species is blank");
            return pet; // Return pet unchanged if validation fails
        }

        logger.info("Pet {} persisted with status: {}", pet.getId(), pet.getStatus());
        return pet;
    }
}

...
public class PetProcessor implements CyodaProcessor {
    ...
    private ErrorInfo handlePetError(Throwable throwable, Pet pet) {
        logger.error("Error processing Pet", throwable);
        return new ErrorInfo("Pet_processing_error", throwable.getMessage());
    }

    private Pet applyBusinessLogic(Pet pet) {
        // Example business logic: update petName to uppercase
        if (pet.getPetName() != null) {
            pet.setPetName(pet.getPetName().toUpperCase());
        }
        return pet;
    }

    private boolean businessValidation(Pet pet) {
        // Example business validation: age must be non-negative
        return pet.getAge() != null && pet.getAge() >= 0;
    }
}

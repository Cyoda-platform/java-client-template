...
public class PetProcessor implements CyodaProcessor {
    ...
    private Pet processEntityLogic(Pet pet) {
        // Actual business logic from processPet() flow in functional_requirement.md
        logger.info("Processing Pet with petId: {}", pet.getPetId());

        if (pet.getStatus() == null) {
            logger.warn("Pet status is null, setting to AVAILABLE");
            pet.setStatus("AVAILABLE");
        }

        if (pet.getAge() != null && pet.getAge() < 0) {
            logger.warn("Pet age is less than 0, resetting to 0");
            pet.setAge(0);
        }

        // Example business rule: Mark pets older than 10 years as ADOPTED
        if (pet.getAge() != null && pet.getAge() > 10) {

...
public class ShipmentReadyProcessor implements CyodaProcessor {
    ...
    public ShipmentReadyProcessor(SerializerFactory serializerFactory, EntityService entityService) {
    ...
    private Shipment processShipmentReady(ProcessorSerializer.ProcessorEntityExecutionContext<Shipment> context) {
        ...
        logger.info("Marking shipment as ready for dispatch: {}", shipment.getShipmentId());

        // Update shipment.lines qtyPicked = qtyOrdered
        ...
        try {
            // Update associated order state to WAITING_TO_SEND
            ...
            if (order != null) {
                ...
                logger.info("Associated order found for shipment ready: orderId={}", shipment.getOrderId());
            } else {
        ...
        logger.info("Shipment marked as ready for dispatch: shipmentId={}, orderId={}", 

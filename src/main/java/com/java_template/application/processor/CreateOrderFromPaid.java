...
/**
 * CreateOrderFromPaid Processor - Creates order from paid cart
 * 
 * Handles CREATE_ORDER_FROM_PAID transition for Order entity.
 * Snapshots cart data to order, decrements product stock, creates shipment.
 */
@Component
public class CreateOrderFromPaid implements CyodaProcessor {
    ...
    /**
     * Main business logic - create order from paid cart
     */
    private EntityWithMetadata<Order> processOrderCreation(
        ...
        try {
            ...
            // Here we perform additional business logic:
            ...
            // 3. Set order status to WAITING_TO_FULFILL
            order.setStatus("WAITING_TO_FULFILL");
            
            // 4. Update timestamp
            order.setUpdatedAt(LocalDateTime.now());

            logger.info("Order {} created successfully from paid cart with {} items", 
                       order.getOrderId(), order.getTotals().getItems());

        } catch (Exception e) {
            logger.error("Error creating order from paid cart: {}", order.getOrderId(), e);
            // In a real system, you might want to set order status to ERROR or similar
        }
    ...
    private void decrementProductStock(Order order) {
        logger.debug("Decrementing product stock for order: {}", order.getOrderId());
        ...
        for (Order.OrderLine line : order.getLines()) {
            try {
                ...
                if (productWithMetadata != null) {
                    ...
                    logger.info("Product {} stock decremented from {} to {} (ordered: {})", 
                ...
                logger.error("Error decrementing stock for product: {} in order: {}", 

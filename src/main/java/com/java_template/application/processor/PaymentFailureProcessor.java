...
public class PaymentFailureProcessor implements CyodaProcessor {
    ...
    public EntityProcessorCalculationResponse process(CyodaEventContext<EntityProcessorCalculationRequest> context) {
        ...
        logger.info("Processing PaymentFailure for request: {}", request.getId());

        return serializer.withRequest(request)
            .toEntity(Order.class)
            .validate(this::isValidEntity, "Invalid order for payment failure handling")
    ...
    private Order processEntityLogic(ProcessorSerializer.ProcessorEntityExecutionContext<Order> context) {
        ...
        // Revert payment status and fulfillment as per policy
        order.setPaymentStatus("PAYMENT_PENDING");
        order.setFulfillmentStatus("CANCELLED");
        logger.info("Order {} payment failed. Marked as CANCELLED", order.getOrderId());
        return order;
    }
}

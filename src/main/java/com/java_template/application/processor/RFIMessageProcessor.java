...
@Component
public class RFIMessageProcessor implements CyodaProcessor {

    private static final Logger logger = LoggerFactory.getLogger(RFIMessageProcessor.class);
    private final String className = this.getClass().getSimpleName();
    private final ProcessorSerializer serializer;

    public RFIMessageProcessor(SerializerFactory serializerFactory) {
    ...
    private EntityWithMetadata<RFI> processRFIMessage(
        ...
        logger.debug("Processing message for RFI: {}", rfi.getRfiId());

        rfi.setUpdatedAt(LocalDateTime.now());
        logger.info("RFI {} message processed", rfi.getRfiId());

...
public class CartCheckoutProcessor implements CyodaProcessor {
    ...
    public EntityProcessorCalculationResponse process(CyodaEventContext<EntityProcessorCalculationRequest> context) {
        ...
        return serializer.withRequest(request)
            ...
            .validate(this::isValidEntity, "Invalid shopping cart for checkout")
    ...
    private ShoppingCart processEntityLogic(ProcessorSerializer.ProcessorEntityExecutionContext<ShoppingCart> context) {
        ...
        try {
            if (cart.getItems() == null || cart.getItems().isEmpty()) {
                logger.warn("Attempted checkout for empty cart {}", cart.getId());
                return cart;
            }
            // Simulate checkout: create an Order snapshot id and mark cart as checked out via updatedAt
            ...
            logger.info("Checkout created order {} for cart {}", order.getId(), cart.getId());
            // In real system, persist and return order id to cart; for now we log.
        } catch (Exception e) {
            logger.error("Error during cart checkout for cart {}: {}", cart != null ? cart.getId() : "<null>", e.getMessage());
        }
        return cart;
    }
}

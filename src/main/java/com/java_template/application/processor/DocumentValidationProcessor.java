...
public class DocumentValidationProcessor implements CyodaProcessor {
    ...
    public EntityProcessorCalculationResponse process(CyodaEventContext<EntityProcessorCalculationRequest> context) {
        ...
        return serializer.withRequest(request)
                ...
                .map(this::processDocumentValidation)
                .complete();
    }
    ...
    private EntityWithMetadata<Document> processDocumentValidation(
        ...
        logger.debug("Processing validation for document: {}", document.getDocumentId());

        document.setUpdatedAt(LocalDateTime.now());
        logger.info("Document {} validation processed", document.getDocumentId());

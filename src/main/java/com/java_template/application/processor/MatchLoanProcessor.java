...
@Component
public class MatchLoanProcessor implements CyodaProcessor {
    private static final Logger logger = LoggerFactory.getLogger(MatchLoanProcessor.class);
    private final String className = this.getClass().getSimpleName();
    private final ProcessorSerializer serializer;

    public MatchLoanProcessor(SerializerFactory serializerFactory) {
    ...
    private EntityWithMetadata<Payment> matchLoanLogic(
        ...
        logger.debug("Matching loan for payment: {}", payment.getPaymentId());

        payment.setMatchedAt(LocalDateTime.now());
        payment.setUpdatedAt(LocalDateTime.now());

        logger.info("Payment {} matched to loan: {}", payment.getPaymentId(), payment.getLoanId());

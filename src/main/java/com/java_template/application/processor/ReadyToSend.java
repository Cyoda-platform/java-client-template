...
/**
 * Processor to mark order and shipment as ready to send
 */
@Component
public class ReadyToSend implements CyodaProcessor {

    private static final Logger logger = LoggerFactory.getLogger(ReadyToSend.class);
    private final String className = this.getClass().getSimpleName();
    private final ProcessorSerializer serializer;
    private final EntityService entityService;
    private final ObjectMapper objectMapper;

    public ReadyToSend(SerializerFactory serializerFactory, EntityService entityService, ObjectMapper objectMapper) {
    ...
    private EntityWithMetadata<Order> processReadyToSend(
        ...
        try {
            ...
            throw new RuntimeException("Failed to process ready to send", e);
        }
    ...
    private void updateShipmentStatus(String orderId, String newStatus) {
        try {
            ...
            if (!shipments.isEmpty()) {
                ...
                entityService.update(shipmentWithMetadata.metadata().getId(), shipment, "ready_to_send");

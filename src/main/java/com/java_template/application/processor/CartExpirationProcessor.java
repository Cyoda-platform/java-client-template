...
@Component
public class CartExpirationProcessor implements CyodaProcessor {

    private static final Logger logger = LoggerFactory.getLogger(CartExpirationProcessor.class);
    private final String className = this.getClass().getSimpleName();
    private final ProcessorSerializer serializer;
    private final EntityService entityService;
    private final ObjectMapper objectMapper;

    public CartExpirationProcessor(SerializerFactory serializerFactory, EntityService entityService, ObjectMapper objectMapper) {
    ...
    public EntityProcessorCalculationResponse process(CyodaEventContext<EntityProcessorCalculationRequest> context) {
        ...
        logger.info("Processing CartExpiration for request: {}", request.getId());

        return serializer.withRequest(request)
            .toEntity(Cart.class)
            .validate(this::isValidEntity, "Invalid cart for expiration")
    ...
    private Cart processEntityLogic(ProcessorSerializer.ProcessorEntityExecutionContext<Cart> context) {
        ...
        // Release reservations: for prototype we just increment inventory back
                    ...
                    if (products != null && products.size() > 0) {
                        ...
                        if (productNode.has("technicalId")) {
                            try {
                                ...
                                logger.warn("Failed to update product inventory during cart expiration for {}", product.getProductId(), ex);
                            }
                        }
                    }
                } catch (InterruptedException | ExecutionException e) {
                    logger.error("Error while releasing inventory for expired cart {}", cart.getCartId(), e);
                }
            }
        }

        cart.setStatus("ABANDONED");
        logger.info("Cart {} marked as ABANDONED and inventory released", cart.getCartId());
        return cart;
    }
}

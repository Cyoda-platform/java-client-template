...
/**
 * BulkUploadRetryProcessor - Retries processing of failed items
 * 
 * Reprocesses only the items that failed in a previous bulk upload attempt.
 */
@Component
public class BulkUploadRetryProcessor implements CyodaProcessor {

    private static final Logger logger = LoggerFactory.getLogger(BulkUploadRetryProcessor.class);
    private final String className = this.getClass().getSimpleName();
    private final ProcessorSerializer serializer;
    private final EntityService entityService;

    public BulkUploadRetryProcessor(SerializerFactory serializerFactory, EntityService entityService) {
    ...
    public EntityProcessorCalculationResponse process(CyodaEventContext<EntityProcessorCalculationRequest> context) {
        ...
        logger.info("Processing BulkUpload retry for request: {}", request.getId());
    ...
    private EntityWithMetadata<BulkUpload> processEntityWithMetadataLogic(
        ...
        logger.debug("Retrying BulkUpload: {}", entity.getUploadId());

        // Reset processing state for retry
        entity.setProcessingStartTime(System.currentTimeMillis());
        Integer previousFailedItems = entity.getFailedItems();
        ...
        for (int i = 0; i < jsonContent.size(); i++) {
            ...
            if (processedCount <= (entity.getTotalItems() - previousFailedItems)) {
                continue;
            }
            ...
            try {
                ...
                logger.warn("Failed to retry item {}: {}", item.getId(), e.getMessage());
            }
        }

        logger.info("BulkUpload {} retry completed - Additional processed: {}, Still failed: {}", 
    ...
    private static class MockFile {
        ...
        public MockFile(String name) { this.name = name; }
    }

    private static class MockJsonContent {
        private final int itemCount;
        public MockJsonContent(int itemCount) { this.itemCount = itemCount; }
    ...
    private static class MockJsonItem {
        ...
        public MockJsonItem(Long id, String type, String by, Long time, String title, String text, String url, Integer score) {
            this.id = id; this.type = type; this.by = by; this.time = time;
            this.title = title; this.text = text; this.url = url; this.score = score;
        }

...
public class SubscriberProcessor implements CyodaProcessor {
    ...
    public EntityProcessorCalculationResponse process(CyodaEventContext<EntityProcessorCalculationRequest> context) {
        ...
        return serializer.withRequest(request)
                ...
                .withErrorHandler(this::handleSubscriberError)
    ...
    public boolean supports(OperationSpecification modelSpec) {
        return "SubscriberProcessor".equals(modelSpec.operationName()) &&
                "subscriber".equals(modelSpec.modelKey().getName()) &&
                Integer.parseInt(Config.ENTITY_VERSION) == modelSpec.modelKey().getVersion();
    }

    private Subscriber applyBusinessLogic(Subscriber subscriber) {
        // Example business logic: update subscribedAt if null
        if (subscriber.getSubscribedAt() == null) {
            subscriber.setSubscribedAt(java.time.Instant.now());
        }
        return subscriber;
    }

    private ErrorInfo handleSubscriberError(Throwable throwable, Subscriber subscriber) {
        logger.error("Error processing Subscriber: {}", throwable.getMessage(), throwable);

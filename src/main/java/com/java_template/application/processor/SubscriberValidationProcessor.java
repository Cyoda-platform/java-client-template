...
public class SubscriberValidationProcessor implements CyodaProcessor {
    ...
    private Subscriber processEntityLogic(ProcessorSerializer.ProcessorEntityExecutionContext<Subscriber> context) {
        ...
        // Business logic from runValidateSubscriber prototype method
        if (subscriber.getContactType() == null || subscriber.getContactType().isBlank()) {
            throw new IllegalArgumentException("contactType is required and cannot be blank");
        }
        if (subscriber.getContactValue() == null || subscriber.getContactValue().isBlank()) {
            throw new IllegalArgumentException("contactValue is required and cannot be blank");
        }

...
@Component
public class MarkJobCompleteProcessor implements CyodaProcessor {

    private static final Logger logger = LoggerFactory.getLogger(MarkJobCompleteProcessor.class);
    private final String className = this.getClass().getSimpleName();
    private final ProcessorSerializer serializer;

    public MarkJobCompleteProcessor(SerializerFactory serializerFactory) {
    ...
    public EntityProcessorCalculationResponse process(CyodaEventContext<EntityProcessorCalculationRequest> context) {
        ...
        logger.info("Marking job complete for request: {}", request.getId());
    ...
    private ImportJob processEntityLogic(ProcessorSerializer.ProcessorEntityExecutionContext<ImportJob> context) {
        ...
        logger.info("Job {} marked COMPLETED", job.getTechnicalId());
        return job;
    }
}

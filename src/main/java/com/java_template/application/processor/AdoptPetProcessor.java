    ...
    private Pet processEntityLogic(ProcessorSerializer.ProcessorEntityExecutionContext<Pet> context) {
        ...
        try {
            ...
            if (res != null) {
                ...
                logger.info("Pet {} marked ADOPTED", pet.getTechnicalId());
            } else {
                logger.warn("AdoptPetProcessor invoked but no reservation present for pet {}", pet.getTechnicalId());
            }
        } catch (Exception e) {
            logger.error("Error during AdoptPetProcessor for pet {}: {}", pet == null ? "<null>" : pet.getTechnicalId(), e.getMessage(), e);
        }
        return pet;
    }
}

...
public class AdoptPetProcessor implements CyodaProcessor {
    ...
    public AdoptPetProcessor(SerializerFactory serializerFactory) {
        ...
        logger.info("AdoptPetProcessor initialized with SerializerFactory");
    }
    ...
    public boolean supports(OperationSpecification modelSpec) {
        return "AdoptPetProcessor".equals(modelSpec.operationName()) &&
                "pet".equals(modelSpec.modelKey().getName()) &&
                Integer.parseInt(Config.ENTITY_VERSION) == modelSpec.modelKey().getVersion();
    }

    private boolean isValidPet(Pet pet) {
        return pet.getStatus() != null && "approved".equalsIgnoreCase(pet.getStatus());
    }

    private Pet applyAdoptLogic(Pet pet) {
        // Example: Set status to adopted
        return new Pet(pet.getId(), pet.getName(), pet.getType(), pet.getAge(), "adopted");
    }

...
public class ScheduleRetryProcessor implements CyodaProcessor {
    ...
    public ScheduleRetryProcessor(SerializerFactory serializerFactory) {
    ...
    public EntityProcessorCalculationResponse process(CyodaEventContext<EntityProcessorCalculationRequest> context) {
        ...
        logger.info("Processing schedule retry for request: {}", request.getId());
    ...
    private Job processEntityLogic(ProcessorSerializer.ProcessorEntityExecutionContext<Job> context) {
        ...
        try {
            // Reschedule retry - set status and increment retry counter if available
            job.setStatus("RETRY_SCHEDULED");
            Integer retries = job.getRetryCount();
            if (retries == null) retries = 0;
            job.setRetryCount(retries + 1);
        } catch (Exception e) {
            logger.error("Error scheduling retry for job {}: {}", job.getTechnicalId(), e.getMessage());
        }
        return job;
    }
}

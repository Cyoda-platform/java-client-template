...
public class DigestRequestProcessor implements CyodaProcessor {
    ...
    private DigestRequest processEntityLogic(DigestRequest entity) {
        // Business logic that was applied after saving entity
        // For example, update status or trigger additional processing
        if (entity.getStatus() == DigestRequest.Status.RECEIVED) {
            entity.setStatus(DigestRequest.Status.PROCESSING);
        }
        return entity;
    }
}

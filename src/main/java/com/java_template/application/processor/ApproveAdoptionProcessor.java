            ...
            try {
                ...
                if (payload != null && payload.getData() != null) {
                    ...
                    if (dataNode.isObject()) {
                        ObjectNode obj = (ObjectNode) dataNode;
                        JsonNode statusNode = obj.get("status");
                        String currentStatus = statusNode == null || statusNode.isNull() ? null : statusNode.asText();
                        ...
                        if (currentStatus == null || currentStatus.isBlank() || "AVAILABLE".equalsIgnoreCase(currentStatus)) {
                            obj.put("status", "PENDING_ADOPTION");
                            // Determine the technical id to use for update (prefer id field in payload if present, otherwise use parsed petUuid)
                            UUID technicalUuid;
                            JsonNode idNode = obj.get("id");
                            if (idNode != null && !idNode.isNull() && !idNode.asText().isBlank()) {
                                try {
                                    technicalUuid = UUID.fromString(idNode.asText());
                                } catch (IllegalArgumentException iae) {
                            ...
                            // Persist the pet update using a generic map to avoid relying on specific Pet setters
                            Map<String, Object> updated = objectMapper.convertValue(obj, Map.class);
                            entityService.updateItem(technicalUuid, updated).get();
                            ...
                            JsonNode idNode = obj.get("id");
                            String petIdText = idNode == null || idNode.isNull() ? petUuid.toString() : idNode.asText();
                            logger.info("Pet {} not in AVAILABLE state (current: {}), skipping status change", petIdText, currentStatus);
                            String existingNotes = entity.getNotes();
                        ...
                        String existing = entity.getNotes();
                        entity.setNotes(existing == null ? note : existing + " ; " + note);
                        logger.warn("Deserialized pet is not an object for petId {}", entity.getPetId());
                    }
                } else {

...
public class MailSendHappyMailProcessor implements CyodaProcessor {
    ...
    private Mail processEntityLogic(ProcessorSerializer.ProcessorEntityExecutionContext<Mail> context) {
        ...
        if (entity.getIsHappy() == null || !entity.getIsHappy()) {
            logger.error("Mail is not marked as happy");
            throw new RuntimeException("Mail is not marked as happy");
        }
        ...
        // Process happy mail sending
        int successCount = 0;
        int failureCount = 0;

        String subject = "🌟 Happy Mail - Brighten Your Day! 🌟";
        String content = generateHappyContent();
        ...
        for (String recipient : entity.getMailList()) {
            if (isValidEmail(recipient)) {
                try {
                    ...
                    if (sendResult) {
                        successCount++;
                        logger.debug("Successfully sent happy mail to: {}", recipient);
                    } else {
                        failureCount++;
                        logger.error("Failed to send happy mail to: {}", recipient);
                    }
                } catch (Exception e) {
                    failureCount++;
                    logger.error("Exception while sending happy mail to {}: {}", recipient, e.getMessage());
                }
            } else {
                failureCount++;
        ...
        if (successCount > 0 && failureCount == 0) {
            logger.info("Happy mail sent successfully to all {} recipients", successCount);
        } else if (successCount > 0) {
            logger.warn("Happy mail partially sent: {} success, {} failed", successCount, failureCount);
        } else {
            logger.error("Failed to send happy mail to any recipient");
            throw new RuntimeException("No recipients received the happy mail");
        }

        return entity;
    }

    private String generateHappyContent() {
        return "Dear Friend,\n\n" +

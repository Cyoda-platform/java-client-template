...
/**
 * MailSendHappyMailProcessor - Processes and sends happy mail to recipients
 * 
 * This processor handles the business logic for sending happy mail to all recipients
 * in the mail list. It validates that the mail is marked as happy and sends
 * appropriate happy content to each recipient.
 */
@Component
public class MailSendHappyMailProcessor implements CyodaProcessor {
    ...
    /**
     * Main business logic processing method for sending happy mail
     * 
    ...
    private EntityWithMetadata<Mail> processEntityWithMetadataLogic(
        ...
        // Validate input - mail must be marked as happy
        if (!Boolean.TRUE.equals(entity.getIsHappy())) {
            logger.error("Mail is not marked as happy - cannot process with happy mail processor");
            throw new IllegalArgumentException("Mail is not marked as happy");
        }
        ...
        String subject = "Happy Mail - Spreading Joy!";
        String content = "Hello! We hope this message brings a smile to your face. Have a wonderful day filled with happiness and joy!";

        // Send mail to each recipient
        for (String email : entity.getMailList()) {
            try {
                sendMail(email, subject, content);
                logger.info("Happy mail sent successfully to: {}", email);
            } catch (Exception e) {
                logger.error("Failed to send happy mail to: {}", email, e);
                throw new RuntimeException("Failed to send mail to " + email, e);
            }
        }

        logger.info("All happy mails sent successfully for entity: {}", currentEntityId);

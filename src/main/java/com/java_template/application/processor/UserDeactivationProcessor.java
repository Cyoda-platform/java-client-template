...
/**
 * UserDeactivationProcessor - Deactivates a user account
 * 
 * Transitions: deactivate_unverified (registered → inactive), 
 *              deactivate_user (active → inactive), 
 *              deactivate_suspended (suspended → inactive)
 * Purpose: Deactivates user account and cancels their orders
 */
@Component
public class UserDeactivationProcessor implements CyodaProcessor {

    private static final Logger logger = LoggerFactory.getLogger(UserDeactivationProcessor.class);
    private final String className = this.getClass().getSimpleName();
    private final ProcessorSerializer serializer;
    private final EntityService entityService;
    private final ObjectMapper objectMapper;

    public UserDeactivationProcessor(SerializerFactory serializerFactory, EntityService entityService, ObjectMapper objectMapper) {
    ...
    public EntityProcessorCalculationResponse process(CyodaEventContext<EntityProcessorCalculationRequest> context) {
        ...
        logger.info("Processing User deactivation for request: {}", request.getId());
    ...
    private boolean isValidEntityWithMetadata(EntityWithMetadata<User> entityWithMetadata) {
        ...
        return entity != null && entity.isValid() && technicalId != null && 
               ("registered".equals(currentState) || "active".equals(currentState) || "suspended".equals(currentState));
    }

    /**
     * Main business logic processing method
     * Deactivates user and cancels their orders
     */
    private EntityWithMetadata<User> processEntityWithMetadataLogic(
        ...
        logger.debug("Processing user deactivation: {} in state: {}", user.getUsername(), currentState);
        ...
        try {
            ...
            for (EntityWithMetadata<Order> orderWithMetadata : orders) {
                ...
                if ("placed".equals(orderState)) {
                    ...
                    logger.debug("Cancelled order {} for deactivated user {}", order.getOrderId(), user.getUsername());
                }
            }
        } catch (Exception e) {
            logger.warn("Could not cancel orders for deactivated user {}: {}", user.getUsername(), e.getMessage());
        }
    }
}

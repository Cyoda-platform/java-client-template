...
@Component
public class TransitionToAvailableProcessor implements CyodaProcessor {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final ProcessorSerializer serializer;

    public TransitionToAvailableProcessor(SerializerFactory serializerFactory) {
        this.serializer = serializerFactory.getDefaultProcessorSerializer();
        logger.info("TransitionToAvailableProcessor initialized with SerializerFactory");
    }
    ...
    public EntityProcessorCalculationResponse process(CyodaEventContext<EntityProcessorCalculationRequest> context) {
        ...
        return serializer.withRequest(request)
                ...
                .map(this::transitionToAvailable)
                .complete();
    }

    @Override
    public boolean supports(OperationSpecification modelSpec) {
        return "TransitionToAvailableProcessor".equals(modelSpec.getOperationName()) &&
                "pet".equals(modelSpec.modelKey().getName()) &&
                Integer.parseInt(Config.ENTITY_VERSION) == modelSpec.modelKey().getVersion();
    }

    private Pet transitionToAvailable(Pet pet) {

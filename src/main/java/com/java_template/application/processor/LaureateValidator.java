...
@Component
public class LaureateValidator implements CyodaProcessor {

    private static final Logger logger = LoggerFactory.getLogger(LaureateValidator.class);
    private final String className = this.getClass().getSimpleName();
    private final ProcessorSerializer serializer;

    public LaureateValidator(SerializerFactory serializerFactory) {
    ...
    public EntityProcessorCalculationResponse process(CyodaEventContext<EntityProcessorCalculationRequest> context) {
        ...
        return serializer.withRequest(request)
            ...
            .validate(this::isValidEntity, "Invalid laureate data")
    ...
    private boolean isValidEntity(Laureate laureate) {
        ...
        if (laureate.getBorn() == null || laureate.getBorn().isEmpty()) {
            logger.error("Born date is null or empty");
            return false;
        }
        if (laureate.getBorncountry() == null || laureate.getBorncountry().isEmpty()) {
            logger.error("Borncountry is null or empty");
            return false;
        }
        if (laureate.getYear() == null || laureate.getYear().isEmpty()) {
            logger.error("Year is null or empty");
            return false;
        }
    ...
    private Laureate processEntityLogic(ProcessorSerializer.ProcessorEntityExecutionContext<Laureate> context) {
        ...
        // No additional processing at validation step
        return laureate;
    }
}

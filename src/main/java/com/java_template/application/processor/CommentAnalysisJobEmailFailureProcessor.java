...
@Component
public class CommentAnalysisJobEmailFailureProcessor implements CyodaProcessor {

    private static final Logger logger = LoggerFactory.getLogger(CommentAnalysisJobEmailFailureProcessor.class);
    private final String className = this.getClass().getSimpleName();
    private final ProcessorSerializer serializer;

    public CommentAnalysisJobEmailFailureProcessor(SerializerFactory serializerFactory) {
    ...
    public EntityProcessorCalculationResponse process(CyodaEventContext<EntityProcessorCalculationRequest> context) {
        ...
        logger.error("Processing email failure for request: {}", request.getId());
    ...
    private EntityWithMetadata<CommentAnalysisJob> processEmailFailure(
        ...
        logger.error("Processing email failure for job: {}", jobId);

        if (job.getErrorMessage() == null || job.getErrorMessage().trim().isEmpty()) {
            job.setErrorMessage("Email sending failed - invalid recipient or service unavailable");
        }

        logger.error("Email sending failed for job {}: {}", jobId, job.getErrorMessage());

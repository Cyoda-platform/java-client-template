...
public class ShipmentDispatchProcessor implements CyodaProcessor {
    ...
    public ShipmentDispatchProcessor(SerializerFactory serializerFactory) {
    ...
    private Shipment processEntityLogic(ProcessorSerializer.ProcessorEntityExecutionContext<Shipment> context) {
        ...
        // Validate all items are picked before dispatch
        for (Shipment.ShipmentLine line : shipment.getLines()) {
            if (line.getQtyPicked() == null || line.getQtyPicked() <= 0) {
                logger.error("All items must be picked before dispatch. SKU: {} has picked qty: {}", 
                           line.getSku(), line.getQtyPicked());
                throw new IllegalStateException("All items must be picked before dispatch");
            }
            
            if (line.getQtyPicked() > line.getQtyOrdered()) {
                logger.error("Picked quantity cannot exceed ordered quantity for SKU: {} - picked: {}, ordered: {}", 
        ...
        // Update shipment lines - set qtyShipped = qtyPicked for all lines
        for (Shipment.ShipmentLine line : shipment.getLines()) {
            line.setQtyShipped(line.getQtyPicked());
            
            logger.debug("Updated shipment line for SKU: {} - shipped qty set to: {}", 
        ...
        logger.info("Shipment dispatched successfully: {} - all picked items shipped", shipment.getShipmentId());

...
public class OrderProcessingProcessor implements CyodaProcessor {
    ...
    public EntityProcessorCalculationResponse process(CyodaEventContext<EntityProcessorCalculationRequest> context) {
        ...
        logger.info("Processing Order processing for request: {}", request.getId());
    ...
    private Order processEntityLogic(ProcessorSerializer.ProcessorEntityExecutionContext<Order> context) {
        ...
        if (!"confirmed".equals(entity.getPaymentStatus())) {
            throw new IllegalStateException("Payment must be confirmed before processing order");
        }

        // Add processing note
        entity.setNotes((entity.getNotes() != null ? entity.getNotes() + " " : "") + 
                       "Order processing started on " + java.time.LocalDateTime.now().toString());

        logger.info("Order processing started: {}", entity.getId());
        return entity;
    }
}

...
public class ArchiveOwnerProcessor implements CyodaProcessor {
    ...
    public EntityProcessorCalculationResponse process(CyodaEventContext<EntityProcessorCalculationRequest> context) {
        ...
        logger.info("Processing ArchiveOwner for request: {}", request.getId());
    ...
    private Owner processEntityLogic(ProcessorSerializer.ProcessorEntityExecutionContext<Owner> context) {
        ...
        try {
            owner.setStatus("ARCHIVED");
            logger.info("Owner {} archived", owner.getTechnicalId());
        } catch (Exception e) {
            logger.error("Error during ArchiveOwnerProcessor for owner {}: {}", owner == null ? "<null>" : owner.getTechnicalId(), e.getMessage(), e);
        }
        return owner;
    }
}

...
public class RevenueRecognitionProcessor implements CyodaProcessor {
    ...
    public RevenueRecognitionProcessor(SerializerFactory serializerFactory, EntityService entityService) {
    ...
    private Opportunity processEntityLogic(ProcessorSerializer.ProcessorEntityExecutionContext<Opportunity> context) {
        ...
        try {
            ...
            if ("CLOSED_WON".equalsIgnoreCase(opp.getStage())) {
                ...
                logger.info("Revenue processed for opportunity {} amount {}", technicalId, opp.getAmount());
                // persist
                if (technicalId != null) {
            ...
            logger.error("Error processing revenue for opportunity {}: {}", technicalId, e.getMessage());
        }
        return opp;
    }
}

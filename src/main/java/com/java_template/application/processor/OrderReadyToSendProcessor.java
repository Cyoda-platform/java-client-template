...
public class OrderReadyToSendProcessor implements CyodaProcessor {
    ...
    private Order processEntityLogic(ProcessorSerializer.ProcessorEntityExecutionContext<Order> context) {
        ...
        logger.info("Marking order ready to send: {}", order.getOrderId());
        ...
        // Find associated shipment and update to WAITING_TO_SEND state
        updateShipmentState(order.getOrderId(), "READY_FOR_SHIPPING");

        // Update order timestamp
        order.setUpdatedAt(LocalDateTime.now());

        logger.info("Order marked ready to send successfully: {}", order.getOrderId());

        return order;
    }

    /**
     * Update the associated shipment state to WAITING_TO_SEND.
     */
    private void updateShipmentState(String orderId, String transition) {
        try {
            logger.info("Updating shipment to ready for shipping for order: {}", orderId);
            ...
            if (shipmentResponse.isPresent()) {
                ...
                // Update shipment with transition (this will move it to WAITING_TO_SEND state)
                entityService.update(entityId, shipment, transition);
                
                logger.info("Shipment updated to ready for shipping: {} for order: {}", 

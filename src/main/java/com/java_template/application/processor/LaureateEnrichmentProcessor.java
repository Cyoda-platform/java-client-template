...
public class LaureateEnrichmentProcessor implements CyodaProcessor {
    ...
    public EntityProcessorCalculationResponse process(CyodaEventContext<EntityProcessorCalculationRequest> context) {
        ...
        logger.info("Enriching Laureate for request: {}", request.getId());

        return serializer.withRequest(request)
            .toEntity(Laureate.class)
            .validate(this::isValidEntity)
            .map(this::processEnrichmentLogic)
            .complete();
    }
    ...
    private Laureate processEnrichmentLogic(ProcessorSerializer.ProcessorEntityExecutionContext<Laureate> context) {
        Laureate laureate = context.entity();

        // Enrichment logic: normalize borncountrycode to uppercase
        if (laureate.getBorncountrycode() != null) {
            laureate.setBorncountrycode(laureate.getBorncountrycode().toUpperCase());
            logger.info("Normalized borncountrycode to uppercase: {}", laureate.getBorncountrycode());
        }

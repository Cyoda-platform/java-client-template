...
@Component
public class SubscriberValidator implements CyodaProcessor {

    private static final Logger logger = LoggerFactory.getLogger(SubscriberValidator.class);
    private final String className = this.getClass().getSimpleName();
    private final ProcessorSerializer serializer;

    public SubscriberValidator(SerializerFactory serializerFactory) {
    ...
    public EntityProcessorCalculationResponse process(CyodaEventContext<EntityProcessorCalculationRequest> context) {
        ...
        return serializer.withRequest(request)
            ...
            .validate(this::isValidEntity, "Invalid subscriber data")
    ...
    private boolean isValidEntity(Subscriber subscriber) {
        ...
        if (subscriber.getContactEmail() == null || subscriber.getContactEmail().isEmpty()) {
            logger.error("ContactEmail is null or empty");
            return false;
        }
        if (subscriber.getActive() == null) {
            logger.error("Active flag is null");
            return false;
        }
        return true;
    }

...
public class SendEmailProcessor implements CyodaProcessor {
    ...
    public EntityProcessorCalculationResponse process(CyodaEventContext<EntityProcessorCalculationRequest> context) {
        ...
        // Work on raw payload (JsonNode) instead of toEntity(Object.class) which violates generics bounds
        return serializer.withRequest(request)
            .map(ctx -> {
                JsonNode payload = ctx.payload();
                ObjectNode deliveryNode = mapper.convertValue(payload, ObjectNode.class);

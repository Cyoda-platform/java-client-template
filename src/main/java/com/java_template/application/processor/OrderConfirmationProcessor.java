...
/**
 * ABOUTME: Processor that handles order confirmation after payment processing,
 * updating payment details and confirming pet reservations.
 */
@Component
public class OrderConfirmationProcessor implements CyodaProcessor {
    ...
    private boolean isValidEntityWithMetadata(EntityWithMetadata<Order> entityWithMetadata) {
        ...
        return order != null && order.isValid(entityWithMetadata.metadata()) && technicalId != null && "placed".equals(currentState);
    }

    /**
     * Main business logic for order confirmation
     */
    private EntityWithMetadata<Order> processEntityWithMetadataLogic(
        ...
        logger.debug("Confirming order: {} for customer: {}", order.getOrderId(), order.getCustomerId());

        // Update confirmation timestamp
        order.setUpdatedAt(LocalDateTime.now());

        // Set payment date if payment info exists
        if (order.getPayment() != null && order.getPayment().getPaymentDate() == null) {
            order.getPayment().setPaymentDate(LocalDateTime.now());
        }

        logger.info("Order {} confirmed successfully for customer: {}", order.getOrderId(), order.getCustomerId());

    ...
    private User processEntityLogic(ProcessorSerializer.ProcessorEntityExecutionContext<User> context) {
        ...
        try {
            if (!"verified".equalsIgnoreCase(user.getStatus())) {
                logger.warn("Cannot activate user {} who is not verified (status={})", user.getId(), user.getStatus());
                return user;
            }
            user.setStatus("active");
            user.setUpdatedAt(Instant.now().toString());
            logger.info("User {} activated", user.getId());
        } catch (Exception e) {
            logger.error("Error in ActivateUserProcessor for user {}: {}", user.getId(), e.getMessage(), e);
        }
        return user;
    }
}

...
@Component
public class PostAccrualEntries implements CyodaProcessor {
    private static final Logger logger = LoggerFactory.getLogger(PostAccrualEntries.class);
    private final String className = this.getClass().getSimpleName();
    private final ProcessorSerializer serializer;

    public PostAccrualEntries(SerializerFactory serializerFactory) {
    ...
    private EntityWithMetadata<Accrual> process(ProcessorSerializer.ProcessorEntityResponseExecutionContext<Accrual> context) {
        ...
        logger.info("PostAccrualEntries completed for entity: {}", entityWithMetadata.metadata().getId());

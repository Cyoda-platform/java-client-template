...
public class OrderMarkSentProcessor implements CyodaProcessor {
    ...
    public OrderMarkSentProcessor(SerializerFactory serializerFactory) {
    ...
    private Order processEntityLogic(ProcessorSerializer.ProcessorEntityExecutionContext<Order> context) {
        ...
        try {
            ...
            // Trigger shipment sent transition
            triggerShipmentMarkSent(order.getOrderId());
            ...
            logger.error("Error processing order mark sent: {}", e.getMessage(), e);
            throw new RuntimeException("Failed to process order mark sent: " + e.getMessage(), e);
        }
    }

    private void triggerShipmentMarkSent(String orderId) {
        try {
            ...
            if (shipment != null) {
                ...
                if (shipmentEntityId != null) {
                    entityService.applyTransition(shipmentEntityId, "MARK_SENT").join();
                    logger.info("Applied MARK_SENT transition to shipment {} for order {}",
                        shipment.getShipmentId(), orderId);
                }
            }
        } catch (Exception e) {
            logger.error("Failed to trigger shipment mark sent for order {}: {}", orderId, e.getMessage(), e);
        }
    }

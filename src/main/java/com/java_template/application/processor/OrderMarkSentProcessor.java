...
public class OrderMarkSentProcessor implements CyodaProcessor {
    ...
    private Order processEntityLogic(ProcessorSerializer.ProcessorEntityExecutionContext<Order> context) {
        ...
        // Find associated shipment and update to SENT state
        updateShipmentState(order.getOrderId(), "DISPATCH");

        // Update order timestamp
        order.setUpdatedAt(LocalDateTime.now());

        logger.info("Order marked as sent successfully: {}", order.getOrderId());

        return order;
    }

    /**
     * Update the associated shipment state to SENT.
     */
    private void updateShipmentState(String orderId, String transition) {
        try {
            logger.info("Updating shipment to sent for order: {}", orderId);
            ...
            if (shipmentResponse.isPresent()) {
                ...
                // Update shipment with transition (this will move it to SENT state)
                entityService.update(entityId, shipment, transition);
                
                logger.info("Shipment updated to sent: {} for order: {}", 

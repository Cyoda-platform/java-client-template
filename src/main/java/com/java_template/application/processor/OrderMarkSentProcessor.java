...
/**
 * Order Mark Sent Processor
 * 
 * Updates shipment when order is sent.
...
public class OrderMarkSentProcessor implements CyodaProcessor {
    ...
    private EntityWithMetadata<Order> processOrderMarkSent(
            ProcessorSerializer.ProcessorEntityResponseExecutionContext<Order> context) {
        
        EntityWithMetadata<Order> entityWithMetadata = context.entityResponse();
        Order order = entityWithMetadata.entity();

        logger.debug("Processing order mark sent: {}", order.getOrderId());
        ...
        try {
            ...
            entityService.update(shipmentResponse.getId(), shipment, "MARK_SENT");
            
            // Update order timestamp
            order.setUpdatedAt(LocalDateTime.now());

            logger.info("Order mark sent processed: {}", order.getOrderId());

            return entityWithMetadata;
        } catch (Exception e) {
            logger.error("Error processing order mark sent", e);

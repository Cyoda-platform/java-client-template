...
/**
 * CompleteAdoptionProcessor - Finalizes adoption and updates related entities
 * 
 * Purpose: Finalize the adoption process
 * Transition: approved -> completed (manual)
 */
@Component
public class CompleteAdoptionProcessor implements CyodaProcessor {
     ...
     * Sets completion date and transitions pet to "adopted" state
     */
    private EntityWithMetadata<Adoption> processEntityWithMetadataLogic(
        ...
        logger.debug("Completing adoption: {}", entity.getAdoptionId());

        // Set adoption.completionDate to current timestamp
        entity.setCompletionDate(LocalDateTime.now());
        logger.info("Set completion date for adoption {} to {}", 
                   entity.getAdoptionId(), entity.getCompletionDate());
        ...
        try {
            ...
            logger.info("Transitioned pet {} to adopted state for adoption {}", 
                       pet.getPetId(), entity.getAdoptionId());
        } catch (Exception e) {
            logger.error("Failed to transition pet {} to adopted state for adoption {}: {}", 
                        pet.getPetId(), entity.getAdoptionId(), e.getMessage());
        }

        logger.info("Adoption {} completed successfully", entity.getAdoptionId());

...
/**
 * Product Entity - Complete schema for persistence and round-trip compatibility
 * 
 * Represents a product in the catalog with complete schema including:
 * - Core required fields (sku, name, description, price, quantityAvailable, category)
 * - Extended schema (attributes, localizations, media, options, variants, bundles, inventory, compliance, relationships, events)
 */
@Data
public class Product implements CyodaEntity {
    ...
    private String sku; // required, unique - Stock Keeping Unit identifier
    private String name; // required - Product display name
    private String description; // required - Product description
    private Double price; // required - Default/base price (fallback)
    private Integer quantityAvailable; // required - Quick projection field for inventory
    private String category; // required - Product category for filtering
